 ...
 * EU VAT Class
 *
 * Handles VAT functionality on WP Ultimo, integrating with our Tax System
 *
 * The code on this particular module is heavily inspired by the work done on the plugin
 * WooCommerce EU VAT Assistant by Aelia - https://wordpress.org/plugins/woocommerce-eu-vat-assistant/
 *
 * @since       2.0.0
 * @author      WP_Ultimo
 * @category    Admin
 * @package     WP_Ultimo/Taxes
 * @version     0.0.1
 */
...
/**
 * Handles VAT rates and compliance.
 *
 * @since 2.0.0
 */
class WU_Tax_EU_VAT {
	...
	/**
	 * Cache key to save the rates retrieved on the database
	 */
	const EU_VAT_RATES_CACHE_KEY = 'wu_eu_vat_rates';

	/**
	 * Cache key used to store the Vat results after a check
	 */
	 ...
	 *
	 * @return object A WU_Tax_EU_VAT instance
	 */
	...
	/**
	 * Adds the necessary hooks and filters for handling EU VAT Taxes rates in WP Ultimo
	 */
	public function __construct() {

		// Add Additional Setting Options to the Tax Section
		add_filter('wu_settings_section_tax', array($this, 'add_settings'), 2);

		/**
     * Bail early if no need for tax support and/or EU VAT support
     */
		if (!WU_Settings::get_setting('enable_tax') || !WU_Settings::get_setting('enable_eu_vat')) {

			return;

		} // end if;

		add_action('wu_load_tax_rates_list_page', array($this, 'enqueue_scripts'));
		...
		add_action('wu_pricing_table_plan_description_line', array($this, 'add_excluding_vat_message'), 9, 2);
	...
	public function validate_vat_number_on_summary(&$checkout) {
		...
		// TODO: We need to refacxtor this here
	...
	public function enqueue_scripts() {
		...
		$min = WU_Scripts()->suffix();

		wp_enqueue_script('wu-vat', WP_Ultimo_VAT()->get_asset("wu-tax-rates-vat$min.js", 'js'), array('jquery', 'wu-tax-rates'), WP_Ultimo_VAT()->version);

	} // end enqueue_scripts;

	/**
	 * Adds a "Excluding VAT" message to the pricing tables.
	 *
	 * @since 2.0.0
	 * @param string  $description The current plan description.
	 * @param WU_Plan $plan The plan for that particular pricing table.
	 * @return string
	 */
	public function add_excluding_vat_message($description, $plan) {

		if (!$plan->free) {

			$description = sprintf('<small>%s</small><br>', __('Excluding VAT', 'wp-ultimo')) . $description;

		} else {

			$description = '<small>--</small><br>' . $description;

		} // end if;

		return $description;

	} // end add_excluding_vat_message;
	 ...
	 * @return void
	 */
	function add_eu_vat_tax_rate_type($types) {
	 ...
	 * @return boolean
	 */
	function is_customer_on_base_country($country) {

		$base_country = WU_Settings::get_setting('company_country', false);
	...
	public function is_customer_on_eu_country($country) {
		...
		return in_array($country, $eu_country_codes);
	 ...
	 * @param string $country_code The countr code.
	 * @return boolean
	 */
	function maybe_exempt_user_on_base_country($applicable_tax_rates, $country_code) {
	 ...
	 * @return void
	 */
	function maybe_add_eu_vat_field_to_signup($fields) {
	...
	/**
	 * Queries all the tax rates in the platform, formats them, and serve them as json
     *
	 * @since 2.0.0
	 * @return void
	 */
	function serve_eu_vat_taxes_rates_via_ajax() {

		$rate_type = isset($_GET['rate_type']) ? $_GET['rate_type'] : 'standard_rate';
		...
		foreach ($eu_vat_tax_rates['rates'] as $country_code => $item) {
			...
			$results[] = array(
				'title'    => sprintf(__('%1$s%% %2$s VAT', 'wp-ultimo'), $rate, $item['country']),
		...
		echo json_encode(apply_filters('wu_get_eu_vat_tax_rates', $results));
	...
	public function get_eu_vat_rates() {
		...
		$vat_rates = get_transient(self::EU_VAT_RATES_CACHE_KEY);
		...
		if ($this->valid_eu_vat_rates($vat_rates['rates'])) {
			...
			set_transient(self::EU_VAT_RATES_CACHE_KEY, $vat_rates, 3 * HOUR_IN_SECONDS);
	...
	public function parse_vat_number($vat_number) {
		...
		if (in_array($prefix, array_values($this->get_eu_vat_country_prefixes()))) {
	...
	public function get_vat_prefix($country) {
		...
		return in_array($country, $country_prefixes) ? $country_prefixes[$country] : false;
	...
	public function set_cache_vat_result($country_code, $vat_number, $valid, $info) {
		...
		return set_transient(self::EU_VAT_CHECK_CACHE_KEY . '_' . $country_code . $vat_number, array(
			'valid'        => $valid,
	...
	function get_cache_vat_result($country_code, $vat_number) {
		...
		return get_transient(self::EU_VAT_CHECK_CACHE_KEY . '_' . $country_code . $vat_number);
	...
	public function validate_vat_number($country_code, $vat_number) {
		...
		if ($cache && $cache['valid']) {
			...
			$vat_validation = new vatValidation(array('debug' => false));
	 ...
	 * @return void
	 */
	function print_eu_vat_actions() { ?>

		<button v-on:click="pullVATData" class="button"><?php _e('Update EU VAT Rates'); ?></button>

		<select v-model="rate_type">
				<option value="standard_rate"><?php echo _e('Use Standard Rates', 'wp-ultimo'); ?></option>
				<option value="reduced_rate"><?php echo _e('Use Reduced Rates', 'wp-ultimo'); ?></option>
				<option value="reduced_rate_alt"><?php echo _e('Use Reduced Rates (Alternative)', 'wp-ultimo'); ?></option>
				<option value="super_reduced_rates"><?php echo _e('Use Super Reduced Rates', 'wp-ultimo'); ?></option>
				<option value="parking_rate"><?php echo _e('Use Parking Rates', 'wp-ultimo'); ?></option>
		</select>
...
}  // end class WU_Tax_EU_VAT;

/**
 * Returns the singleton
 */
function WU_Tax_EU_VAT() { // phpcs:ignore

	return WU_Tax_EU_VAT::get_instance();
