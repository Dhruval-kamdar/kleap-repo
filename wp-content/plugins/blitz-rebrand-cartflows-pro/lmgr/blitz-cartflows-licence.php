<?php 
namespace BZ_FLOWS\Admin\License;
use BZ_FLOWS\Admin\FLOWAdminSettings;

define('BZFLOW_APP_API_URL',  'https://waas-pro.com/index.php');


class BZFLOW_License {
 	...
 	public function init($productID=null) {
 		...
 		$this->productId = BZFLOW_PRODUCT_ID;
 	...
 	public function bzflows_license_status() {
 		...
 		//auto updater start
			Blitz_run_updater("https://waas-pro.com/index.php", 'blitz-rebrand-cartflows-pro',BZFLOW_PLUGIN_FILE,BZFLOW_VERSION,$license_key1,$this->productId);
		//auto updater end
		
		return $this->bzflows_license_check($license_key1,'status-check');
		
 	}



 	public function bzflows_license_menu() {
	 		
	 		add_submenu_page(
		            'settings.php',
		            __('Rebrand LifterLMS License', 'my-plugin-domain'),
		            __('Rebrand LifterLMS License'),
		            'manage_options',
		            'glmlicense',
		            array($this, 'bzflows_licenseBlock')
		        );    
 	...
 	public function bzflows_license_check($license_key,$action) {
			...
			$request_uri    = BZFLOW_APP_API_URL . '?' . http_build_query( $args );
 	...
 	public function bzflows_licenseBlock() {
			...
			if (isset($_REQUEST['activate_license'])) {
					...
					$response = $this->bzflows_license_check($license_key,'activate');
					echo $response[1];
					if($response[0]){
						if(is_multisite() ){
							if( is_plugin_active_for_network(BZFLOW_PLUGIN_FILE) ) {
			...
			if (isset($_REQUEST['deactivate_license'])) {
					//~ $license_key = $_REQUEST[$this->productField];
					$response = $this->bzflows_license_check($license_key1,'deactivate');
					echo $response[1];
					if($response[0]){
						if(is_multisite() ){
							if( is_plugin_active_for_network(BZFLOW_PLUGIN_FILE) ) {
			...
			//~ $responsecheck = $this->bzflows_license_check($license_key1,'status-check');
			
			if( $license_key1 != '' ) {
				$adminSettings = new FLOWAdminSettings(BZFLOW_PRODUCT_ID);
				$responsecheck = $adminSettings->bzflows_valid_license();
			} else {
				$responsecheck = False;
			}
