...
{
    if ($promiseOrValue instanceof \Beehive\React\Promise\ExtendedPromiseInterface) {
    ...
    if (\is_object($promiseOrValue) && \method_exists($promiseOrValue, 'then')) {
        ...
        return new \Beehive\React\Promise\Promise(function ($resolve, $reject, $notify) use($promiseOrValue) {
            $promiseOrValue->then($resolve, $reject, $notify);
        }, $canceller);
    }
    return new \Beehive\React\Promise\FulfilledPromise($promiseOrValue);
}
/**
...
{
    if ($promiseOrValue instanceof \Beehive\React\Promise\PromiseInterface) {
        return resolve($promiseOrValue)->then(function ($value) {
            return new \Beehive\React\Promise\RejectedPromise($value);
        });
    }
    return new \Beehive\React\Promise\RejectedPromise($promiseOrValue);
}
/**
...
{
    $cancellationQueue = new \Beehive\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \Beehive\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $cancellationQueue) {
...
function some($promisesOrValues, $howMany)
{
    $cancellationQueue = new \Beehive\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \Beehive\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $howMany, $cancellationQueue) {
        resolve($promisesOrValues)->done(function ($array) use($howMany, $cancellationQueue, $resolve, $reject, $notify) {
            ...
            $len = \count($array);
            if ($len < $howMany) {
                throw new \Beehive\React\Promise\Exception\LengthException(\sprintf('Input array must contain at least %d item%s but contains only %s item%s.', $howMany, 1 === $howMany ? '' : 's', $len, 1 === $len ? '' : 's'));
            }
            $toResolve = $howMany;
...
function map($promisesOrValues, callable $mapFunc)
{
    $cancellationQueue = new \Beehive\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \Beehive\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $mapFunc, $cancellationQueue) {
...
function reduce($promisesOrValues, callable $reduceFunc, $initialValue = null)
{
    $cancellationQueue = new \Beehive\React\Promise\CancellationQueue();
    $cancellationQueue->enqueue($promisesOrValues);
    return new \Beehive\React\Promise\Promise(function ($resolve, $reject, $notify) use($promisesOrValues, $reduceFunc, $initialValue, $cancellationQueue) {
