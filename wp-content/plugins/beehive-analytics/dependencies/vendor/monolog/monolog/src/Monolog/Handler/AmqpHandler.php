...
use AMQPExchange;
class AmqpHandler extends \Beehive\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, $exchangeName = 'log', $level = \Beehive\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if ($exchange instanceof \AMQPExchange) {
            $exchange->setName($exchangeName);
        } elseif ($exchange instanceof \Beehive\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    {
        return new \Beehive\PhpAmqpLib\Message\AMQPMessage((string) $data, array('delivery_mode' => 2, 'content_type' => 'application/json'));
    }
    /**
     * {@inheritDoc}
     */
