...
class Google_Http_Batch
{
    ...
    private $rootUrl;
    private $batchPath;
    public function __construct(\Beehive\Google_Client $client, $boundary = \false, $rootUrl = null, $batchPath = null)
    {
        $this->client = $client;
    ...
    public function add(\Beehive\Psr\Http\Message\RequestInterface $request, $key = \false)
    {
        if (\false == $key) {
            $key = \mt_rand();
        }
...
EOF;
        ...
        $request = new \Beehive\GuzzleHttp\Psr7\Request('POST', $url, $headers, $body);
        $response = $this->client->execute($request);
        return $this->parseResponse($response, $classes);
    }
    public function parseResponse(\Beehive\Psr\Http\Message\ResponseInterface $response, $classes = array())
    {
        ...
        if (!empty($body)) {
            ...
            foreach ($parts as $i => $part) {
                ...
                if (!empty($part)) {
                    ...
                    $response = new \Beehive\GuzzleHttp\Psr7\Response($status, $partHeaders, \Beehive\GuzzleHttp\Psr7\stream_for($partBody));
                    // Need content id.
                    $key = $headers['content-id'];
                    try {
                        $response = \Beehive\Google_Http_REST::decodeHttpResponse($response, $requests[$i - 1]);
                    } catch (\Beehive\Google_Service_Exception $e) {
 ...
 * class.
 */
\class_alias('Beehive\\Google_Http_Batch', 'Google_Http_Batch', \false);
