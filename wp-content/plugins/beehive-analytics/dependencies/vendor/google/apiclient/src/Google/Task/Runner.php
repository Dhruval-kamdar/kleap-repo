...
class Google_Task_Runner
{
    ...
    public function __construct($config, $name, $action, array $arguments = array())
    {
        if (isset($config['initial_delay'])) {
            if ($config['initial_delay'] < 0) {
                throw new \Beehive\Google_Task_Exception('Task configuration `initial_delay` must not be negative.');
            }
            $this->delay = $config['initial_delay'];
        }
        if (isset($config['max_delay'])) {
            if ($config['max_delay'] <= 0) {
                throw new \Beehive\Google_Task_Exception('Task configuration `max_delay` must be greater than 0.');
            }
            $this->maxDelay = $config['max_delay'];
        }
        if (isset($config['factor'])) {
            if ($config['factor'] <= 0) {
                throw new \Beehive\Google_Task_Exception('Task configuration `factor` must be greater than 0.');
            }
            $this->factor = $config['factor'];
        }
        if (isset($config['jitter'])) {
            if ($config['jitter'] <= 0) {
                throw new \Beehive\Google_Task_Exception('Task configuration `jitter` must be greater than 0.');
            }
            $this->jitter = $config['jitter'];
        }
        if (isset($config['retries'])) {
            if ($config['retries'] < 0) {
                throw new \Beehive\Google_Task_Exception('Task configuration `retries` must not be negative.');
            }
            $this->maxAttempts += $config['retries'];
        }
        if (!\is_callable($action)) {
            throw new \Beehive\Google_Task_Exception('Task argument `$action` must be a valid callable.');
        }
        $this->action = $action;
    ...
    public function run()
    {
        while ($this->attempt()) {
            ...
            } catch (\Beehive\Google_Service_Exception $exception) {
 ...
 * @see https://developers.google.com/drive/web/handle-errors#implementing_exponential_backoff
 */
\class_alias('Beehive\\Google_Task_Runner', 'Google_Task_Runner', \false);
