...
use Beehive\Stash\Driver\FileSystem;
use Beehive\Stash\Pool;
/**
...
class Google_AccessToken_Verify
{
     ...
     */
    public function __construct(\Beehive\GuzzleHttp\ClientInterface $http = null, \Beehive\Psr\Cache\CacheItemPoolInterface $cache = null, $jwt = null)
    {
        if (null === $http) {
            $http = new \Beehive\GuzzleHttp\Client();
        }
        if (null === $cache) {
            $cache = new \Beehive\Google\Auth\Cache\MemoryCacheItemPool();
        }
        $this->http = $http;
        $this->cache = $cache;
    ...
    private function retrieveCertsFromLocation($url)
    {
        ...
        if (0 !== \strpos($url, 'http')) {
            if (!($file = \file_get_contents($url))) {
                throw new \Beehive\Google_Exception("Failed to retrieve verification certificates: '" . $url . "'.");
            }
        ...
        throw new \Beehive\Google_Exception(\sprintf('Failed to retrieve verification certificates: "%s".', $response->getBody()->getContents()), $response->getStatusCode());
    }
 ...
 *
 */
\class_alias('Beehive\\Google_AccessToken_Verify', 'Google_AccessToken_Verify', \false);
