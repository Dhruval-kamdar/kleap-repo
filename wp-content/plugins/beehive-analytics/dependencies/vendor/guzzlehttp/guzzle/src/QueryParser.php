...
class QueryParser
{
     ...
     */
    public function parseInto(\Beehive\GuzzleHttp\Query $query, $str, $urlEncoding = \true)
    {
        if ($str === '') {
            return;
        }
        ...
        if (!$this->numericIndices) {
            $query->setAggregator(\Beehive\GuzzleHttp\Query::phpAggregator(\false));
        } elseif ($this->duplicates) {
            $query->setAggregator(\Beehive\GuzzleHttp\Query::duplicateAggregator());
        }
    }
    /**
    ...
    private static function getDecoder($type)
    {
        ...
        } elseif ($type == \Beehive\GuzzleHttp\Query::RFC3986) {
            return 'rawurldecode';
        } elseif ($type == \Beehive\GuzzleHttp\Query::RFC1738) {
            return 'urldecode';
        } else {
            return function ($str) {
                return $str;
