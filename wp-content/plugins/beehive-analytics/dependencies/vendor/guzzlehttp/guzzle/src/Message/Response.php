...
use Beehive\GuzzleHttp\Exception\ParseException;
use Beehive\GuzzleHttp\Exception\XmlParseException;
use Beehive\GuzzleHttp\Stream\StreamInterface;
use Beehive\GuzzleHttp\Utils;
/**
 * Guzzle HTTP response object
 */
class Response extends \Beehive\GuzzleHttp\Message\AbstractMessage implements \Beehive\GuzzleHttp\Message\ResponseInterface
{
     ...
     */
    public function __construct($statusCode, array $headers = [], \Beehive\GuzzleHttp\Stream\StreamInterface $body = null, array $options = [])
    {
    ...
    {
        try {
            return \Beehive\GuzzleHttp\Utils::jsonDecode((string) $this->getBody(), isset($config['object']) ? !$config['object'] : \true, 512, isset($config['big_int_strings']) ? \JSON_BIGINT_AS_STRING : 0);
        } catch (\InvalidArgumentException $e) {
            throw new \Beehive\GuzzleHttp\Exception\ParseException($e->getMessage(), $this);
        }
    }
    public function xml(array $config = [])
    {
        ...
        try {
            ...
            \libxml_disable_entity_loader($disableEntities);
            \libxml_use_internal_errors($internalErrors);
            throw new \Beehive\GuzzleHttp\Exception\XmlParseException('Unable to parse response body into XML: ' . $e->getMessage(), $this, $e, \libxml_get_last_error() ?: null);
        }
        return $xml;
    }
