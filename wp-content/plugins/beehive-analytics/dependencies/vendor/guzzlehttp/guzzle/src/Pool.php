...
use Beehive\React\Promise\Deferred;
use Beehive\React\Promise\FulfilledPromise;
use Beehive\React\Promise\PromiseInterface;
 ...
 * performance.
 */
class Pool implements \Beehive\GuzzleHttp\Ring\Future\FutureInterface
{
     ...
     */
    public function __construct(\Beehive\GuzzleHttp\ClientInterface $client, $requests, array $options = [])
    {
        $this->client = $client;
        $this->iter = $this->coerceIterable($requests);
        $this->deferred = new \Beehive\React\Promise\Deferred();
     ...
     */
    public static function batch(\Beehive\GuzzleHttp\ClientInterface $client, $requests, array $options = [])
    {
        ...
        (new self($client, $requests, \Beehive\GuzzleHttp\Event\RequestEvents::convertEventArray($options, ['end'], ['priority' => \Beehive\GuzzleHttp\Event\RequestEvents::LATE, 'fn' => function (\Beehive\GuzzleHttp\Event\EndEvent $e) use($hash) {
            $hash[$e->getRequest()] = $e->getException() ? $e->getException() : $e->getResponse();
        }])))->wait();
        return new \Beehive\GuzzleHttp\BatchResults($hash);
    }
    /**
     ...
     *                                  {@see GuzzleHttp\Pool::__construct}
     */
    public static function send(\Beehive\GuzzleHttp\ClientInterface $client, $requests, array $options = [])
    {
    ...
    private function coerceIterable($requests)
    {
        ...
        throw new \InvalidArgumentException('Expected Iterator or array. ' . 'Found ' . \Beehive\GuzzleHttp\Ring\Core::describeType($requests));
    }
    /**
    ...
    private function addNextRequest()
    {
        ...
        $this->iter->next();
        if (!$request instanceof \Beehive\GuzzleHttp\Message\RequestInterface) {
            throw new \InvalidArgumentException(\sprintf('All requests in the provided iterator must implement ' . 'RequestInterface. Found %s', \Beehive\GuzzleHttp\Ring\Core::describeType($request)));
        }
        ...
        $request->getEmitter()->on('before', [$this, '_trackRetries'], \Beehive\GuzzleHttp\Event\RequestEvents::EARLY);
        $response = $this->client->send($request);
        $this->waitQueue[$hash] = $response;
        $promise = $response->promise();
        // Don't recursively call itself for completed or rejected responses.
        if ($promise instanceof \Beehive\React\Promise\FulfilledPromise || $promise instanceof \Beehive\React\Promise\RejectedPromise) {
            try {
    ...
    public function _trackRetries(\Beehive\GuzzleHttp\Event\BeforeEvent $e)
    {
    ...
    private function finishResponse($request, $value, $hash)
    {
        ...
        $result = $value instanceof \Beehive\GuzzleHttp\Message\ResponseInterface ? ['request' => $request, 'response' => $value, 'error' => null] : ['request' => $request, 'response' => null, 'error' => $value];
