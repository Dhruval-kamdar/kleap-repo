 ...
 * Post file upload
 */
class PostFile implements \Beehive\GuzzleHttp\Post\PostFileInterface
{
    private $name;
    private $filename;
    private $content;
    ...
    private function prepareContent($content)
    {
        ...
        if (!$this->content instanceof \Beehive\GuzzleHttp\Stream\StreamInterface) {
            $this->content = \Beehive\GuzzleHttp\Stream\Stream::factory($this->content);
        } elseif ($this->content instanceof \Beehive\GuzzleHttp\Post\MultipartBody) {
    ...
    private function prepareDefaultHeaders()
    {
        ...
        if (!$this->hasHeader('Content-Type')) {
            $this->headers['Content-Type'] = \Beehive\GuzzleHttp\Mimetypes::getInstance()->fromFilename($this->filename) ?: 'text/plain';
        }
    }
    /**
