...
class Url
{
    ...
    public static function fromString($url)
    {
        ...
        if ($parts['query'] || 0 !== \strlen($parts['query'])) {
            $parts['query'] = \Beehive\GuzzleHttp\Query::fromString($parts['query']);
        }
    ...
    {
        if ($this->query instanceof \Beehive\GuzzleHttp\Query) {
    ...
    public function getQuery()
    {
        ...
        if (!$this->query instanceof \Beehive\GuzzleHttp\Query) {
            $this->query = $this->query === null ? new \Beehive\GuzzleHttp\Query() : \Beehive\GuzzleHttp\Query::fromString($this->query);
        }
        return $this->query;
    }
    /**
    ...
    {
        if ($query instanceof \Beehive\GuzzleHttp\Query) {
            $this->query = $query;
        } elseif (\is_string($query)) {
            if (!$rawString) {
                $this->query = \Beehive\GuzzleHttp\Query::fromString($query);
            } else {
            ...
            $this->query = new \Beehive\GuzzleHttp\Query($query);
        } else {
            throw new \InvalidArgumentException('Query must be a Query, ' . 'array, or string. Got ' . \Beehive\GuzzleHttp\Ring\Core::describeType($query));
        }
    }
    /**
    ...
    public function combine($url)
    {
        ...
        if ($parts['host']) {
            return new static($this->scheme, $parts['host'], $parts['user'], $parts['pass'], $parts['port'], $parts['path'], $parts['query'] instanceof \Beehive\GuzzleHttp\Query ? clone $parts['query'] : $parts['query'], $parts['fragment']);
        }
        ...
        $result = new self($this->scheme, $this->host, $this->username, $this->password, $this->port, $path, $query instanceof \Beehive\GuzzleHttp\Query ? clone $query : $query, $parts['fragment']);
        if ($path) {
