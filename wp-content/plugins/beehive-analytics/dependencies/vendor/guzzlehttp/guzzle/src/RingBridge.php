...
use Beehive\GuzzleHttp\Event\ProgressEvent;
use Beehive\GuzzleHttp\Message\Request;
...
class RingBridge
{
     ...
     */
    public static function createRingRequest(\Beehive\GuzzleHttp\Message\RequestInterface $request)
    {
     ...
     * @return array Converted Guzzle Ring request.
     */
    public static function prepareRingRequest(\Beehive\GuzzleHttp\Transaction $trans)
    {
        ...
        if ($trans->request->getEmitter()->hasListeners('progress')) {
            ...
            $request['client']['progress'] = function ($a, $b, $c, $d) use($trans, $emitter) {
                $emitter->emit('progress', new \Beehive\GuzzleHttp\Event\ProgressEvent($trans, $a, $b, $c, $d));
            };
        }
        return $request;
    }
     ...
     */
    public static function completeRingResponse(\Beehive\GuzzleHttp\Transaction $trans, array $response, \Beehive\GuzzleHttp\Message\MessageFactoryInterface $messageFactory)
    {
    ...
    public static function fromRingRequest(array $request)
    {
        ...
        return new \Beehive\GuzzleHttp\Message\Request($request['http_method'], \Beehive\GuzzleHttp\Ring\Core::url($request), isset($request['headers']) ? $request['headers'] : [], isset($request['body']) ? \Beehive\GuzzleHttp\Stream\Stream::factory($request['body']) : null, $options);
    }
    /**
     ...
     */
    public static function getNoRingResponseException(\Beehive\GuzzleHttp\Message\RequestInterface $request)
    {
        $message = <<<EOT
...
EOT;
        return new \Beehive\GuzzleHttp\Exception\RequestException($message, $request);
    }
}
