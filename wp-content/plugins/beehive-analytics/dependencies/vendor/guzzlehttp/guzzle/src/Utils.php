...
use Beehive\GuzzleHttp\Ring\Client\CurlHandler;
use Beehive\GuzzleHttp\Ring\Client\CurlMultiHandler;
use Beehive\GuzzleHttp\Ring\Client\StreamHandler;
use Beehive\GuzzleHttp\Ring\Client\Middleware;
/**
...
final class Utils
{
    ...
    {
        if (\function_exists('Beehive\\uri_template')) {
            return \Beehive\uri_template($template, $variables);
        }
        static $uriTemplate;
        if (!$uriTemplate) {
            $uriTemplate = new \Beehive\GuzzleHttp\UriTemplate();
        }
    ...
    public static function getDefaultUserAgent()
    {
        static $defaultAgent = '';
        if (!$defaultAgent) {
            $defaultAgent = 'Guzzle/' . \Beehive\GuzzleHttp\ClientInterface::VERSION;
    ...
    public static function getDefaultHandler()
    {
        ...
        if (\extension_loaded('curl')) {
            ...
            if (\function_exists('curl_reset')) {
                $default = new \Beehive\GuzzleHttp\Ring\Client\CurlHandler();
                $future = new \Beehive\GuzzleHttp\Ring\Client\CurlMultiHandler($config);
            } else {
                $default = new \Beehive\GuzzleHttp\Ring\Client\CurlMultiHandler($config);
            }
        }
        if (\ini_get('allow_url_fopen')) {
            $default = !$default ? new \Beehive\GuzzleHttp\Ring\Client\StreamHandler() : \Beehive\GuzzleHttp\Ring\Client\Middleware::wrapStreaming($default, new \Beehive\GuzzleHttp\Ring\Client\StreamHandler());
        } elseif (!$default) {
            throw new \RuntimeException('Guzzle requires cURL, the ' . 'allow_url_fopen ini setting, or a custom HTTP handler.');
        }
        return $future ? \Beehive\GuzzleHttp\Ring\Client\Middleware::wrapFuture($default, $future) : $default;
    }
}
