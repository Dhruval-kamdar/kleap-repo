...
use Beehive\GuzzleHttp\Stream\Exception\CannotAttachException;
 ...
 */
class PostBody implements \Beehive\GuzzleHttp\Post\PostBodyInterface
{
    /** @var StreamInterface */
    private $body;
    /** @var callable */
     ...
     */
    public function applyRequestHeaders(\Beehive\GuzzleHttp\Message\RequestInterface $request)
    {
    ...
    public function getFields($asString = \false)
    {
        ...
        $query = new \Beehive\GuzzleHttp\Query($this->fields);
        $query->setEncodingType(\Beehive\GuzzleHttp\Query::RFC1738);
    ...
    {
        throw new \Beehive\GuzzleHttp\Stream\Exception\CannotAttachException();
    }
    public function eof()
    {
    ...
    private function getBody()
    {
        if ($this->body) {
            ...
            return $this->body = \Beehive\GuzzleHttp\Stream\Stream::factory();
        }
    }
    /**
    ...
    {
        if (!$this->aggregator) {
            $this->aggregator = \Beehive\GuzzleHttp\Query::phpAggregator();
        }
    ...
    private function createMultipart()
    {
        ...
        return new \Beehive\GuzzleHttp\Post\MultipartBody(\call_user_func($this->getAggregator(), $this->fields), $this->files);
    }
    /**
    ...
    {
        return \Beehive\GuzzleHttp\Stream\Stream::factory($this->getFields(\true));
    }
    /**
