...
use Beehive\GuzzleHttp\Ring\Exception\ConnectException;
use Beehive\GuzzleHttp\Ring\Exception\RingException;
use Beehive\GuzzleHttp\Ring\Future\CompletedFutureArray;
use Beehive\GuzzleHttp\Stream\InflateStream;
use Beehive\GuzzleHttp\Stream\StreamInterface;
use Beehive\GuzzleHttp\Stream\Stream;
use Beehive\GuzzleHttp\Stream\Utils;
/**
...
class StreamHandler
{
    ...
    {
        $url = \Beehive\GuzzleHttp\Ring\Core::url($request);
        \Beehive\GuzzleHttp\Ring\Core::doSleep($request);
        try {
            // Does not support the expect header.
            $request = \Beehive\GuzzleHttp\Ring\Core::removeHeader($request, 'Expect');
            $stream = $this->createStream($url, $request);
            return $this->createResponse($request, $url, $stream);
        } catch (\Beehive\GuzzleHttp\Ring\Exception\RingException $e) {
    ...
    private function createResponse(array $request, $url, $stream)
    {
        ...
        $response = ['version' => \substr($parts[0], 5), 'status' => $parts[1], 'reason' => isset($parts[2]) ? $parts[2] : null, 'headers' => \Beehive\GuzzleHttp\Ring\Core::headersFromLines($hdrs), 'effective_url' => $url];
        ...
        return new \Beehive\GuzzleHttp\Ring\Future\CompletedFutureArray($response);
    }
    private function checkDecode(array $request, array $response, $stream)
    {
        // Automatically decode responses when instructed.
        if (!empty($request['client']['decode_content'])) {
            switch (\Beehive\GuzzleHttp\Ring\Core::firstHeader($response, 'Content-Encoding', \true)) {
                case 'gzip':
                case 'deflate':
                    $stream = new \Beehive\GuzzleHttp\Stream\InflateStream(\Beehive\GuzzleHttp\Stream\Stream::factory($stream));
                    break;
            }
        }
        return $stream;
    ...
    private function drain($stream, $dest)
    {
        if (\is_resource($stream)) {
            if (!\is_resource($dest)) {
                $stream = \Beehive\GuzzleHttp\Stream\Stream::factory($stream);
            } else {
        ...
        $dest = \is_string($dest) ? new \Beehive\GuzzleHttp\Stream\Stream(\Beehive\GuzzleHttp\Stream\Utils::open($dest, 'r+')) : \Beehive\GuzzleHttp\Stream\Stream::factory($dest);
        \Beehive\GuzzleHttp\Stream\Utils::copyToStream($stream, $dest);
        $dest->seek(0);
        $stream->close();
        return $dest;
    }
     ...
     * @return array
     */
    private function createErrorResponse($url, \Beehive\GuzzleHttp\Ring\Exception\RingException $e)
    {
        // Determine if the error was a networking error.
        $message = $e->getMessage();
        // This list can probably get more comprehensive.
        if (\strpos($message, 'getaddrinfo') || \strpos($message, 'Connection refused')) {
            $e = new \Beehive\GuzzleHttp\Ring\Exception\ConnectException($e->getMessage(), 0, $e);
        }
        return new \Beehive\GuzzleHttp\Ring\Future\CompletedFutureArray(['status' => null, 'body' => null, 'headers' => [], 'effective_url' => $url, 'error' => $e]);
    }
    /**
    ...
    private function createResource(callable $callback)
    {
        ...
        if (!$resource) {
            ...
            throw new \Beehive\GuzzleHttp\Ring\Exception\RingException(\trim($message));
        }
        return $resource;
    }
    private function createStream($url, array $request)
    {
        ...
        if ((!isset($request['version']) || $request['version'] == '1.1') && !\Beehive\GuzzleHttp\Ring\Core::hasHeader($request, 'Connection')) {
    ...
    private function getDefaultOptions(array $request)
    {
        ...
        $body = \Beehive\GuzzleHttp\Ring\Core::body($request);
        if (isset($body)) {
            $context['http']['content'] = $body;
            // Prevent the HTTP handler from adding a Content-Type header.
            if (!\Beehive\GuzzleHttp\Ring\Core::hasHeader($request, 'Content-Type')) {
    ...
    private function add_verify(array $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \Beehive\GuzzleHttp\Ring\Client\ClientUtils::getDefaultCaBundle();
            }
        } elseif (\is_string($value)) {
            $options['ssl']['cafile'] = $value;
            if (!\file_exists($value)) {
                throw new \Beehive\GuzzleHttp\Ring\Exception\RingException("SSL CA bundle not found: {$value}");
            }
        } elseif ($value === \false) {
            ...
            throw new \Beehive\GuzzleHttp\Ring\Exception\RingException('Invalid verify request option');
        }
    ...
    private function add_cert(array $request, &$options, $value, &$params)
    {
        ...
        if (!\file_exists($value)) {
            throw new \Beehive\GuzzleHttp\Ring\Exception\RingException("SSL certificate not found: {$value}");
        }
    ...
    private function add_progress(array $request, &$options, $value, &$params)
    {
        ...
        $params['notification'] = isset($params['notification']) ? \Beehive\GuzzleHttp\Ring\Core::callArray([$params['notification'], $fn]) : $fn;
    }
    private function add_debug(array $request, &$options, $value, &$params)
    {
        ...
        $value = \Beehive\GuzzleHttp\Ring\Core::getDebugResource($value);
        $ident = $request['http_method'] . ' ' . \Beehive\GuzzleHttp\Ring\Core::url($request);
    ...
    private function applyCustomOptions(array $request, array &$options)
    {
        ...
        if (!\is_array($request['client']['stream_context'])) {
            throw new \Beehive\GuzzleHttp\Ring\Exception\RingException('stream_context must be an array');
        }
