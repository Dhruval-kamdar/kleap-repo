...
use Beehive\GuzzleHttp\Ring\Future\FutureArray;
...
class CurlMultiHandler
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $this->factory = isset($options['handle_factory']) ? $options['handle_factory'] : new \Beehive\GuzzleHttp\Ring\Client\CurlFactory();
    ...
    public function __invoke(array $request)
    {
        ...
        $entry = ['request' => $request, 'response' => [], 'handle' => $result[0], 'headers' => &$result[1], 'body' => $result[2], 'deferred' => new \Beehive\React\Promise\Deferred()];
        $id = (int) $result[0];
        $future = new \Beehive\GuzzleHttp\Ring\Future\FutureArray($entry['deferred']->promise(), [$this, 'execute'], function () use($id) {
    ...
    private function processMessages()
    {
        while ($done = \curl_multi_info_read($this->_mh)) {
            ...
            $result = \Beehive\GuzzleHttp\Ring\Client\CurlFactory::createResponse($this, $entry['request'], $entry['response'], $entry['headers'], $entry['body']);
