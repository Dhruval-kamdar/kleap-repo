...
use Beehive\GuzzleHttp\Stream\LazyOpenStream;
...
class CurlFactory
{
    ...
    public static function createResponse(callable $handler, array $request, array $response, array $headers, $body)
    {
        ...
        if (!empty($headers)) {
            ...
            $headerList = \Beehive\GuzzleHttp\Ring\Core::headersFromLines($headers);
            ...
            $response['body'] = $body;
            \Beehive\GuzzleHttp\Ring\Core::rewindBody($response);
        }
    ...
    private static function createErrorResponse(callable $handler, array $request, array $response)
    {
        ...
        $error = isset($response['curl']['errno']) && isset($connectionErrors[$response['curl']['errno']]) ? new \Beehive\GuzzleHttp\Ring\Exception\ConnectException($message) : new \Beehive\GuzzleHttp\Ring\Exception\RingException($message);
    ...
    private function applyMethod(array $request, array &$options)
    {
        ...
        switch ($request['http_method']) {
            ...
            case 'POST':
                ...
                if (!\Beehive\GuzzleHttp\Ring\Core::hasHeader($request, 'Content-Length')) {
    ...
    {
        $contentLength = \Beehive\GuzzleHttp\Ring\Core::firstHeader($request, 'Content-Length');
        ...
        if ($size !== null && $size < 1000000 || isset($request['client']['curl']['body_as_string']) || \is_string($request['body'])) {
            $options[\CURLOPT_POSTFIELDS] = \Beehive\GuzzleHttp\Ring\Core::body($request);
        ...
        if (!\Beehive\GuzzleHttp\Ring\Core::hasHeader($request, 'Expect')) {
    ...
    {
        $body = $request['body'];
        if ($body instanceof \Beehive\GuzzleHttp\Stream\StreamInterface) {
    ...
    private function applyHeaders(array $request, array &$options)
    {
        ...
        if (!\Beehive\GuzzleHttp\Ring\Core::hasHeader($request, 'Accept')) {
        ...
        foreach ($request['client'] as $key => $value) {
            switch ($key) {
                ...
                case 'decode_content':
                    ...
                    $accept = \Beehive\GuzzleHttp\Ring\Core::firstHeader($request, 'Accept-Encoding');
                    if ($accept) {
                ...
                case 'save_to':
                    if (\is_string($value)) {
                        ...
                        $value = new \Beehive\GuzzleHttp\Stream\LazyOpenStream($value, 'w+');
                    }
                    if ($value instanceof \Beehive\GuzzleHttp\Stream\StreamInterface) {
                ...
                case 'debug':
                    if ($value) {
                        $options[\CURLOPT_STDERR] = \Beehive\GuzzleHttp\Ring\Core::getDebugResource($value);
    ...
    private static function retryFailedRewind(callable $handler, array $request, array $response)
    {
        ...
        if (!\Beehive\GuzzleHttp\Ring\Core::rewindBody($request)) {
