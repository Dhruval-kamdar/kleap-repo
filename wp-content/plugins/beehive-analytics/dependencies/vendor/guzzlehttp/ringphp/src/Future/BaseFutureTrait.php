...
use Beehive\GuzzleHttp\Ring\Exception\CancelledFutureAccessException;
...
trait BaseFutureTrait
{
     ...
     */
    public function __construct(\Beehive\React\Promise\PromiseInterface $promise, callable $wait = null, callable $cancel = null)
    {
    ...
    public function wait()
    {
        if (!$this->isRealized) {
            ...
            if (!$this->isRealized) {
                $this->error = new \Beehive\GuzzleHttp\Ring\Exception\RingException('Waiting did not resolve future');
            }
        }
        if ($this->error) {
            throw $this->error;
    ...
    public function cancel()
    {
        if (!$this->isRealized) {
            ...
            $this->isRealized = \true;
            $this->error = new \Beehive\GuzzleHttp\Ring\Exception\CancelledFutureAccessException();
            if ($cancelfn) {
                $cancelfn($this);
            }
        }
