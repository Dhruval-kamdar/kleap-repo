 ...
 */
class Stream implements \Beehive\GuzzleHttp\Stream\StreamInterface
{
    private $stream;
    private $size;
    private $seekable;
    ...
    public static function factory($resource = '', array $options = [])
    {
        ...
        if ($resource instanceof \Beehive\GuzzleHttp\Stream\StreamInterface) {
            return $resource;
        }
        ...
        if (\is_callable($resource)) {
            return new \Beehive\GuzzleHttp\Stream\PumpStream($resource, $options);
        }
        if ($resource instanceof \Iterator) {
            return new \Beehive\GuzzleHttp\Stream\PumpStream(function () use($resource) {
                if (!$resource->valid()) {
                    return \false;
                }
