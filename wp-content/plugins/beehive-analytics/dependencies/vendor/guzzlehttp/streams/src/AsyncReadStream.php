 ...
 */
class AsyncReadStream implements \Beehive\GuzzleHttp\Stream\StreamInterface
{
     ...
     *                                   writable.
     */
    public function __construct(\Beehive\GuzzleHttp\Stream\StreamInterface $buffer, array $config = [])
    {
    ...
    public static function create(array $options = [])
    {
        ...
        if ($maxBuffer === 0) {
            $buffer = new \Beehive\GuzzleHttp\Stream\NullStream();
        } elseif (isset($options['buffer'])) {
            $buffer = $options['buffer'];
        } else {
            $hwm = isset($options['hwm']) ? $options['hwm'] : 16384;
            $buffer = new \Beehive\GuzzleHttp\Stream\BufferStream($hwm);
        }
        if ($maxBuffer > 0) {
            $buffer = new \Beehive\GuzzleHttp\Stream\DroppingStream($buffer, $options['max_buffer']);
        }
        // Call the on_write callback if an on_write function was provided.
        if (isset($options['write'])) {
            $onWrite = $options['write'];
            $buffer = \Beehive\GuzzleHttp\Stream\FnStream::decorate($buffer, ['write' => function ($string) use($buffer, $onWrite) {
