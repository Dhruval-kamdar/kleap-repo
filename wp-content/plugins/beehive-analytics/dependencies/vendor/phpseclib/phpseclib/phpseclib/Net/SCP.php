...
class SCP
{
    ...
    {
        if ($ssh instanceof \Beehive\phpseclib\Net\SSH2) {
            $this->mode = self::MODE_SSH2;
        } elseif ($ssh instanceof \Beehive\phpseclib\Net\SSH1) {
    ...
    function put($remote_file, $data, $mode = self::SOURCE_STRING, $callback = null)
    {
        ...
        if ($this->mode == self::MODE_SSH2) {
            $this->packet_size = $this->ssh->packet_size_client_to_server[\Beehive\phpseclib\Net\SSH2::CHANNEL_EXEC] - 4;
        }
    ...
    function _send($data)
    {
        switch ($this->mode) {
            case self::MODE_SSH2:
                $this->ssh->_send_channel_packet(\Beehive\phpseclib\Net\SSH2::CHANNEL_EXEC, $data);
                break;
            case self::MODE_SSH1:
    ...
    function _receive()
    {
        switch ($this->mode) {
            case self::MODE_SSH2:
                return $this->ssh->_get_channel_packet(\Beehive\phpseclib\Net\SSH2::CHANNEL_EXEC, \true);
            case self::MODE_SSH1:
                if (!$this->ssh->bitmap) {
                    return \false;
                }
                while (\true) {
                    ...
                    switch ($response[\Beehive\phpseclib\Net\SSH1::RESPONSE_TYPE]) {
                        case NET_SSH1_SMSG_STDOUT_DATA:
                            if (\strlen($response[\Beehive\phpseclib\Net\SSH1::RESPONSE_DATA]) < 4) {
                                return \false;
                            }
                            \extract(\unpack('Nlength', $response[\Beehive\phpseclib\Net\SSH1::RESPONSE_DATA]));
                            return $this->ssh->_string_shift($response[\Beehive\phpseclib\Net\SSH1::RESPONSE_DATA], $length);
    ...
    function _close()
    {
        switch ($this->mode) {
            case self::MODE_SSH2:
                $this->ssh->_close_channel(\Beehive\phpseclib\Net\SSH2::CHANNEL_EXEC, \true);
                break;
            case self::MODE_SSH1:
                $this->ssh->disconnect();
        }
