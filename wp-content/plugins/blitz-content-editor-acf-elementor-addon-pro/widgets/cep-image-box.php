...
class Widget_Cep_Image_Box extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'Content Editor: Image Box', 'elementor' );
	}
	...
	protected function render() {
    	...
    	if ( 'image_short' === $settings['graphic_element'] ) {
		...
		if ( ! empty( $settings['image_short']['url'] ) ) {
			$this->add_render_attribute( 'image_short', 'src', $settings['image_short']['url'] );
			$this->add_render_attribute( 'image_short', 'alt', Control_Media::get_image_alt( $settings['image_short'] ) );
			$this->add_render_attribute( 'image_short', 'title', Control_Media::get_image_title( $settings['image_short'] ) );

			if ( $settings['hover_animation'] ) {
				$this->add_render_attribute( 'image_short', 'class', 'elementor-animation-' . $settings['hover_animation'] );
			}

			$image_html = Group_Control_Image_Size::get_attachment_image_html( $settings, 'thumbnail', 'image_short' );
	...
	protected function _content_template() {
		...
		if ( 'image_short' === settings.graphic_element ) { 
			...
			if ( settings.image_short.url ) {
				var image = {
					id: settings.image_short.id,
					url: settings.image_short.url,
					size: settings.thumbnail_size,
					dimension: settings.thumbnail_custom_dimension,
					model: view.getEditModel()
				};

				var image_url = elementor.imagesManager.getImageUrl( image_short );
