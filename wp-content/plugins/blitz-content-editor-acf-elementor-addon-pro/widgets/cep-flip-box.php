...
class Widget_Cep_Flip_Box extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'Content Editor: Flip Box', 'elementor-pro' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'short_or_image',
			[
				'label' => __( 'Media or Shortcode', 'elementor-pro' ),
				'type' => Controls_Manager::CHOOSE,
				'label_block' => false,
				'options' => [
					'flip_image_short' => [
		...
		$this->add_control(
			'image_short',
			[
				'label' => __( 'Image Shortcode', 'elementor-pro' ),
				'type' => Controls_Manager::TEXT,
				'condition' => [
					'graphic_element' => 'image',
					'short_or_image' => 'flip_image_short',
				],
			]
		);
		...
		$this->add_control(
			'icon_short',
			[
				'label' => __( 'Icon Shortcode', 'elementor-pro' ),
				'type' => Controls_Manager::TEXT,
				'condition' => [
					'graphic_element' => 'icon',
					'short_or_icon' => 'flip_icon_short',
				],
			]
		);
	...
	protected function render() {
		...
		if ( 'image' === $settings['graphic_element'] ) {
			...
			if ( 'flip_image_short' === $settings['short_or_image'] ) {
			
				$image_ID = do_shortcode($settings['image_short']);
				$image_URL = wp_get_attachment_url( $image_ID );
				
				foreach($settings as $settingKey => $settingVal) {
					if($settingKey == 'image_short') {
					 $img['url'] = $image_URL;
					 $img['id'] = $image_ID;
					 $settings['image']=$img;
					}
			...
			} else if ( 'flip_image' === $settings['short_or_image']  ) {
		...
		if ( 'icon' === $settings['graphic_element'] ) {
			...
			if ( 'flip_icon_short' === $settings['short_or_icon'] ) {
				$key = do_shortcode($settings['icon_short']);
				$settings['icon']= 'fa '.$key;
				$settings['icon_short']= 'fa '.$key;
			} else if ( 'flip_icon' === $settings['short_or_icon']  ) {
			...
			<div class="elementor-flip-box__layer elementor-flip-box__front">
				<div class="elementor-flip-box__layer__overlay">
					<div class="elementor-flip-box__layer__inner">
						<?php if ( 'image' === $settings['graphic_element'] && ! empty( $settings['image'] ) ) : ?>
							<div class="elementor-flip-box__image">
								<?php echo Group_Control_Image_Size::get_attachment_image_html( $settings ); ?>
							</div>
						<?php elseif ( 'icon' === $settings['graphic_element'] 	) : ?>
							<div <?php echo $this->get_render_attribute_string( 'icon-wrapper' ); ?>>
								<div class="elementor-icon">
									<?php 
										
									if( 'flip_icon' === $settings['short_or_icon'] 	)  {
											
											$icon = $this->get_render_attribute_string( 'icon' );

											if (strpos($icon, 'fa ') !== false) {
											 $icon = $icon;
											} else {
											 $icon1 = str_replace('class="',' ',$icon);
											 $icon2 = str_replace('"',' ',$icon1);
											 $icon = 'class="fa '.trim($icon2).'"';
											}
