...
class Widget_Cep_Testimonial extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'Content Editor: Testimonial', 'elementor' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'testimonial_short',
			[
				'label' => __( 'Icon Shortcode', 'elementor' ),
				'type' => Controls_Manager::TEXT,
				'placeholder' => '[short type=fieldtype item=0]',
				'condition' => [
					'graphic_element' => 'testimonial_short',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'condition' => [
					'graphic_element' => 'testimonial_image',
				],
			]
		);
	...
	protected function render() {
		...
		if ( 'testimonial_short' === $settings['graphic_element'] ) {

			$image_ID = do_shortcode($settings['testimonial_short']);
			$image_URL = wp_get_attachment_url($image_ID);
		
			foreach($settings as $settingKey => $settingVal) {
				
				if($settingKey == 'testimonial_short') {
				 $img['url'] = $image_URL;
				 $img['id'] = $image_ID;
				 $settings['testimonial_image']=$img;
				}
			}
