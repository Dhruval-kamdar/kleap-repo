...
namespace BZ_ACF_WZ\Admin\License;
use BZ_ACF_WZ\Admin\BZ_ACFWidgetadminSettings;

define('BZ_ACF_WZ_APP_API_URL',  'https://waas-pro.com/index.php');

class BZ_ACFWidgetLicenceManager{
 	...
 	public function init($productID=null)
 	{
 		...
 		$this->productId = BZ_ACF_WZ_SL_PRODUCT_ID;
 		$this->productField = 'software_license_key_' . $this->productId;
 	}

 	public function bz_acf_wz_license_status()
 	{
 		$license_key1 = get_site_option($this->productField);

		//auto updater start
			Blitz_run_updater("https://waas-pro.com/index.php", 'blitz-content-editor-acf-elementor-addon-pro',BZ_ACF_WZ_EL_FILE,BZ_ACF_WZ_EL_VERSION,$license_key1,$this->productId);
		//auto updater end
		
		return $this->bz_acf_wz_license_check($license_key1,'status-check');
 	}

 	public function bz_acf_wz_license_menu() {
	 		
	 		add_submenu_page(
		            'settings.php',
		            __('Web Settings PRO License', 'my-plugin-domain'),
		            __('Web Settings PRO License'),
		            'manage_options',
		            'websettingslicense',
		            array($this, 'bz_acf_wz_licenseBlock')
		        );    
		}

 	public function bz_acf_wz_license_check($license_key,$action) {
			...
			$request_uri    = BZ_ACF_WZ_APP_API_URL . '?' . http_build_query( $args );
 	...
 	public function bz_acf_wz_licenseBlock() {
			echo '<div class="wrap">';
			...
			if (isset($_REQUEST['activate_license'])) {
					...
					$response = $this->bz_acf_wz_license_check($license_key,'activate');
					echo $response[1];
					if($response[0]){
			...
			if (isset($_REQUEST['deactivate_license'])) {
					//~ $license_key = $_REQUEST[$this->productField];
					$response = $this->bz_acf_wz_license_check($license_key1,'deactivate');
					echo $response[1];
					if($response[0]){
						update_site_option($this->productField, ''); 
						update_site_option('cep_elem_addon_license_expired',''); 
					}
			}
			...
			if (isset($_REQUEST['reset_license'])) {
					...
					if( $license_key1 != '' ) {
						...
						if($response[0]){
							if(is_multisite() ){
								if( is_plugin_active_for_network(BZ_ACF_WZ_EL_FILE) ) {
									update_site_option($this->productField, ''); 
									update_site_option('cep_elem_addon_license', ''); 
									update_site_option('cep_elem_addon_license_expired', ''); 
								} else {
									update_option($this->productField, ''); 
									update_option('cep_elem_addon_license', ''); 
									update_option('cep_elem_addon_license_expired', ''); 
								}
							} else {
								update_option($this->productField, ''); 
								update_option('cep_elem_addon_license', ''); 
			...
			if( $license_key1 != '' ) {
				$adminSettings = new BZ_ACFWidgetadminSettings(BZ_ACF_WZ_SL_PRODUCT_ID);
				$responsecheck = $adminSettings->bz_acf_wz_valid_license();
			} else {
				$responsecheck = False;
			}
			
			//~ $responsecheck = $this->bz_acf_wz_license_check($license_key1,'status-check');
