...
/**
 * REST API: PH_REST_Comments_Controller class
 *
 * @package WordPress
 * @subpackage REST_API
 * @since 4.7.0
 ...
 */
class PH_REST_Comments_Controller extends WP_REST_Controller
{
	 ...
	 * @var string
	 */
	public $namespace = 'projecthuddle/v2';

	/**
	 * Default to mockup comment
	 * @var string
	 */
	public $rest_base = '';
	 ...
	 * @var string
	 */
	protected $comment_type = 'ph_comment';
	...
	public function __construct($args = array())
	{
		...
		// disable flood filter
		add_filter('comment_flood_filter', array($this, 'disable_flood_filter'), 10, 3);

		// allow duplicate comments for our comment type
		add_filter('duplicate_comment_id', array($this, 'allow_duplicates'), 10, 2);
	}
	...
	public function get_items_permissions_check($request)
	{
		if (!empty($request['post'])) {
			foreach ((array) $request['post'] as $post_id) {
				if (!Post::get($post_id)) {
					return new WP_Error('invalid_post_id', __('Invalid post id.'), array('status' => rest_authorization_required_code()));
				}

				if ($request['_signature']) {
					$data = new PH_Permissions_Data(Post::get($post_id)->projectId(), false, $request['_signature']);
					$checker = new PH_Signature_Checker($data);
					if ($checker->validate()) {
						return true;
					}
				}

				// can the current visitor read the post?
				if (!Visitor::current()->canAccess(Post::get($post_id))) {
					return new WP_Error('rest_cannot_read_post', __('Sorry, you are not allowed to access this project.'), array('status' => rest_authorization_required_code()));
				}
			}
		}
		...
		if (!isset($project_id) && $request['project']) {
			$project_id = $request['project'];
		}

		// if the user is not a member, don't let them query this
		if (isset($post_id) && !ph_user_is_member($post_id)) {
			...
			foreach ($protected_params as $param) {
				if ('status' === $param) {
					...
					if ($request[$param]) {
						if ('comment' !== $request[$param] && !in_array($request[$param], apply_filters('ph_comment_type_query_types', array('comment', 'approval', 'assignment', 'timeline', 'all')))) {
							$forbidden_params[] = $param;
						}
					}
			...
			if (!empty($forbidden_params)) {
				return new WP_Error('rest_forbidden_param', sprintf(__('You are not allowed to do that.'), implode(', ', $forbidden_params)), array('status' => rest_authorization_required_code()));
			}
		}
	...
	public function get_items($request)
	{
		...
		$parameter_mappings = array(
			...
			'search'         => 'search',
			'status'         => 'status',
			'type'           => 'type', // type to return (maybe map these?)
		);
		...
		// only our comment types
		if (isset($registered['type__in']) && empty($request['type'])) {
			$prepared_args['type__in'] = ph_get_comment_types();
		} else {
			switch ($request['type']) {
				case 'comment':
					$normalized = array('ph_comment');
					break;
				case 'approval':
					$normalized = array('ph_approval');
					break;
				case 'assignment':
					$normalized = array('ph_assign');
					break;
				case 'timeline':
					$normalized = ph_get_timeline_comment_types();
					break;
				default:
					$normalized = ph_get_comment_types();
					break;
			}

			$prepared_args['type__in'] = apply_filters('ph_comment_type_query_types', $normalized, $request);
		}
		...
		// if we haven't specified, or have specifically requested project members
		if (isset($request['project_member']) && filter_var($request['project_member'], FILTER_VALIDATE_BOOLEAN)) {
			if (!isset($prepared_args['meta_query'])) {
				$prepared_args['meta_query'] = array();
			}
		...
		// if querying by item
		if (isset($registered['item'], $request['item']) && $request['item']) {
			if (!isset($prepared_args['meta_query'])) {
				$prepared_args['meta_query'] = array();
			}
			$prepared_args['meta_query'][] = array(
				'key'   => 'item_id',
				'value'   => (array) $request['item'],
				'compare' => 'IN',
			);
		}
	...
	public function create_item_permissions_check($request)
	{
		...
		$project_id = ph_get_project_id_from_post_id($post->ID);
		$project    = get_post($project_id);

		if (!$this->check_project_access_permission($project, $request)) {
		...
		// we should always be able to comment on our post types
		if (!comments_open($post->ID)) {

			if (in_array(get_post_type($post->ID), ph_get_child_post_types())) {
				return true;
			}
			if (in_array(get_post_type($post->ID), ph_get_post_types())) {
				return true;
			}

			// 403 if not our post type
	...
	public function create_item($request)
	{
		...
		// force approval for our comment type endpoint
		$prepared_comment['comment_approved'] = 1;

		// force our comment type on save
		if ($this->comment_type) {
			$prepared_comment['comment_type'] = $this->comment_type;
		}
	...
	public function update_item($request)
	{
		...
		if (empty($prepared_args) && isset($request['status'])) {
			 ...
			 * @since 4.8.0 $prepared_comment can now be a WP_Error to shortcircuit insertion.
			 *
			 * @param array|WP_Error  $prepared_args The prepared comment args for wp_update_comment().
			 * @param WP_REST_Request $request          Request used to insert the comment.
			 */
			$prepared_args = apply_filters('rest_pre_update_comment', $prepared_args, $request);
		...
		// store project/item ids in comment meta
		$parents = ph_get_parents_ids($comment);
		if ($parents['project']) {
			update_comment_meta($id, 'project_id', $parents['project']);
		}
		if ($parents['item']) {
			update_comment_meta($id, 'item_id', $parents['item']);
		}
	...
	public function prepare_item_for_response($comment, $request)
	{
		...
		// append parent id and item id
		$data['project_id'] = $comment->project_id ? (int) $comment->project_id : 0;
		$data['item_id']    = $comment->item_id ? (int) $comment->item_id : 0;
	...
	protected function prepare_links($comment)
	{
		...
		if ($assigned) {
			$links['assigned'] = array(
				'href'       => rest_url('projecthuddle/v2/users/' . $assigned),
				'embeddable' => true,
			);
		}
		...
		if (0 !== (int) $comment->comment_post_ID) {
			...
			if (!empty($post->ID)) {
				...
				$links['up'] = array(
					'href'       => rest_url('projecthuddle/v2/' . $base . '/' . $comment->comment_post_ID),
					'embeddable' => true,
		...
		if ($parents['item']) {
			if (isset($parents['mockup-image'])) {
				$item_base = 'mockup-image';
			}
			if (isset($parents['website-page'])) {
				$item_base = 'website-page';
			}

			if ($item_base) {
				$links['item'] = array(
					'href'       => rest_url('projecthuddle/v2/' . $item_base . '/' . $parents['item']),
					'embeddable' => true,
					'post_type'  => $item_base,
				);
			}
		...
		if ($parents['project']) {
			if (isset($parents['mockup'])) {
				$project_base = 'mockup';
			}
			if (isset($parents['website'])) {
				$project_base = 'website';
			}
			if ($project_base) {
				$links['project'] = array(
					'href'       => rest_url('projecthuddle/v2/' . $project_base . '/' . $parents['project']),
					'embeddable' => true,
					'post_type'  => $project_base,
				);
			}
		}
	...
	public function get_item_schema()
	{
		$schema = array(
			...
			'properties' => array(
				...
				'item'              => array(
					'description' => __('The ID of the associated post item.'),
					'type'        => 'integer',
					'context'     => array('view', 'edit'),
					'default'     => 0,
					'readonly'    => true,
				...
				'project'           => array(
					'description' => __('The ID of the associated post project object.'),
					'type'        => 'integer',
					'context'     => array('view', 'edit'),
					'default'     => 0,
					'readonly'    => true,
	...
	public function get_collection_params()
	{
		...
		$query_params['item'] = array(
			'default'     => array(),
			'description' => __('Limit result set to comments of specific item IDs.'),
			'type'        => 'array',
			'items'       => array(
				'type' => 'integer',
			),
	...
	{
		$posts_controller = new PH_REST_Posts_Controller($post->post_type);
		...
		if (post_password_required($post)) {
			...
			$result = $posts_controller->check_read_permission($post, $request);
		}
		...
		if ($result) {
			User::current()->can('read_post', Thread::get($post->ID));
			// make sure visitor can access
			$validator = new PH_Permissions_Controller($post->ID, '', $request['ph_access_token']);
	...
	{
		if ($request['_signature']) {
			$data = new PH_Permissions_Data(Post::get($comment->comment_post_ID)->projectId(), false, $request['_signature']);
			$checker = new PH_Signature_Checker($data);
			if ($checker->validate()) {
				return true;
			}
		...
		// can the current visitor read the thread?
		if (Visitor::current()->canAccess(Post::get($comment->comment_post_ID))) {
			return true;
		}
	...
	protected function check_edit_permission($comment)
	{
		...
		// let them edit their own comment, if it's our comment type
		if ('ph_website_comment' === $comment->comment_type || 'ph_image_comment' === $comment->comment_type) {
			// No cap required to edit your own comment
			if ((int) get_current_user_id() === (int) $comment->user_id) {
				return true;
			}
		}

		// let them edit if they can moderate_comments
	...
	/**
	 * Disable comment flood filter if current user can edit threads
	 *
	 * @param boolean $block
	 ...
	 * @return bool
	 */
	public function disable_flood_filter($block, $time_lastcomment, $time_newcomment)
	{
		// disable flood filter if user can edit a child post type
		foreach (ph_get_child_post_types() as $post_type) {
	...
	/**
	 * Allow duplicate comments for comments on our website pages
	 *
	 * @param $dupe_id     integer Duplicate comment id
	 * @param $commentdata array Comment data
	 *
	 * @return bool
	 */
	public function allow_duplicates($dupe_id, $commentdata)
	{
		if (in_array(get_post_type($commentdata['comment_post_ID']), ph_get_child_post_types())) {
			return false;
		}
		return $dupe_id;
	}
