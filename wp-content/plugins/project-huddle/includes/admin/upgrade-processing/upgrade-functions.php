...
function ph_update_members()
{
	...
	ph_log('Successfully created members database');
	...
	ph_log('Added all members in table');
...
function ph_thread_members_create_table()
{
	...
	$sql = "CREATE TABLE `{$wpdb->prefix}ph_thread_members` (
		  user_id bigint(20) UNSIGNED NOT NULL,
	...
	ph_log('Successfully created thread members database');
...
function ph_maybe_logout_all_users()
{
	if (version_compare(get_option('ph_db_version'), '3.8.10', '>=')) {
		return;
	}
...
// add_action('init', 'ph_maybe_logout_all_users');
