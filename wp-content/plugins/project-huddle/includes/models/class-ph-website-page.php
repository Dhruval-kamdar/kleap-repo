		...
		register_rest_field(
			...
			array(
				...
				'get_callback'    => function ($post, $attr, $request, $object_type) {
					...
					$current_website_url = get_post_meta($project_id, 'website_url', true);

					// if url has changed, dynamicaly update.
					if (trailingslashit($website_url) !== trailingslashit($current_website_url)) {
						// get project ID.
						$project_id = get_post_meta($post['id'], 'parent_id', true);
						...
						$data = str_replace($website_url, $current_website_url, $data);
					}
