...
class WU_Admin_Pages_List_Table extends WP_List_Table {
	...
	function get_replace_mode_label($replace_mode) {

		$labels = array(
			'all'           => __('Replacing the entire %s page contents', 'wu-apc'),
			'append_top'    => __('Appending content to the top of the %s page', 'wu-apc'),
			'append_bottom' => __('Appending content to the bottom of the %s page', 'wu-apc'),
		);
	...
	function column_menu_parent($item) {
		...
		if ($item->menu_type == 'submenu') {
			...
			if (strpos($item->menu_parent, 'wuapc') !== false) {
				...
				return WU_Admin_Pages::get_instance()->get_parent_name($item->menu_parent);

			} // end if;

		} elseif ($item->menu_type == 'replace') {

			$type = $item->replace_mode;

			return sprintf($this->get_replace_mode_label($item->replace_mode), WU_Admin_Pages::get_instance()->get_parent_name($item->page_to_replace));
