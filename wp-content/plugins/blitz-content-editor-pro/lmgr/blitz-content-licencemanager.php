...
define('CE_SL_APP_API_URL',  'https://waas-pro.com/index.php');

class Blitz_Content_LicenceManager{
 	...
 	public function init($productId=null)
 	{
 		...
 		$this->productId = ($productId==null) ? SL_PRODUCT_ID_CE : $productId;
 	...
 	public function license_status()
 	{
 		...
 		//auto updater start
			Blitz_run_updater("https://waas-pro.com/index.php", 'blitz-content-editor-pro', 'blitz-content-editor-pro/blitz-content-editor-pro.php',CE_VERSION,$license_key1,$this->productId);
		//auto updater end
 	...
 	public function license_check($license_key,$action) {
			...
			$request_uri    = CE_SL_APP_API_URL . '?' . http_build_query( $args );
 	...
 	public function licenseBlock() {
			...
			if (isset($_REQUEST['deactivate_license'])) {
					...
					if($response[0]){
						...
						update_site_option('cep_license_expired', ''); 
					}
			}
			...
			if (isset($_REQUEST['reset_license'])) {
					...
					if( $license_key1 != '' ) {
						...
						if($response[0]){
							if(is_multisite() ){
								if( is_plugin_active_for_network(CE_PLUGIN_FILE) ) {
									update_site_option($this->productField, ''); 
									update_site_option('cep_license', ''); 
									update_site_option('cep_license_expired', ''); 
								} else {
									update_option($this->productField, ''); 
									update_option('cep_license', ''); 
									update_option('cep_license_expired', ''); 
								}
							} else {
								update_option($this->productField, ''); 
								update_option('cep_license', ''); 
			...
			if( $license_key1 != '' ) {
				$adminSettings = new ceadminSettings();
				$responsecheck = $adminSettings->ce_valid_license();
			} else {
				$responsecheck = False;
			}
