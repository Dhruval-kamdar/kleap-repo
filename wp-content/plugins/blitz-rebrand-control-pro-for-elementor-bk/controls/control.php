...
function elementor_controls_load_plugin() {
	load_plugin_textdomain( 'blitz-controls-for-elementor' );
...
function elementor_controls_fail_load() {
	...
	if ( _is_elementor_installed() ) {
		...
		$message = '<p>' . __( 'Blitz Controls Pro not working because you need to activate the Elementor plugin.', 'blitz-controls-for-elementor' ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $activation_url, __( 'Activate Elementor Now', 'blitz-controls-for-elementor' ) ) . '</p>';
	} else {
		...
		$message = '<p>' . __( 'Blitz Controls Pro is not working because you need to install the Elementor plugin', 'blitz-controls-for-elementor' ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $install_url, __( 'Install Elementor Now', 'blitz-controls-for-elementor' ) ) . '</p>';
	}
...
function elementor_controls_fail_load_out_of_date() {
	...
	$message = '<p>' . __( 'Blitz Controls Pro not working because you are using an old version of Elementor.', 'blitz-controls-for-elementor' ) . '</p>';
	$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Elementor Now', 'blitz-controls-for-elementor' ) ) . '</p>';
...
function elementor_controls_admin_notice_upgrade_recommendation() {
	...
	$upgrade_link = wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' ) . $file_path, 'upgrade-plugin_' . $file_path );
	$message = '<p>' . __( 'A new version of Elementor is available. For better performance and compatibility of Elementor Custom Controls, we recommend updating to the latest version.', 'blitz-controls-for-elementor' ) . '</p>';
	$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Elementor Now', 'blitz-controls-for-elementor' ) ) . '</p>';
...
if ( is_multisite()) {
    $options = get_blog_option( 1,$section );
}
else{
