...
function erp_hr_assets_get_all( $args = [] ) {
    ...
    $result = wp_cache_get( $cache_key, 'erp-asset' );

    if ( false == $result ) {
        $result = $wpdb->get_results( $sql );
    }

    wp_cache_set( $cache_key, $result, 'erp-asset' );
    ...
    foreach ( $result as $single ) {
        ...
        $assets[] = [
            ...
            'available_item_total' => !$single->available_item_total ? 'stock' == $single->status ? 1 : 0 : intval( $single->available_item_total )
        ];
    }
...
function erp_hr_allottment_get_all( $args ) {
    ...
    $result     = wp_cache_get( $cache_key, 'erp-allottment' );

    if ( false == $result ) {
        $result     = $wpdb->get_results( $sql );
    }

    wp_cache_set( $cache_key, $result, 'erp-allottment' );
...
function erp_hr_asset_request_all( $args ) {
    ...
    if ( isset( $args['status'] ) && !empty( $args['status'] ) ) {
        $sql .= " WHERE req.status = '" . $args['status'] . "'";
    }
    ...
    $result     = wp_cache_get( $cache_key, 'erp-asset-request' );
    ...
    foreach ( $result as $single ) {
        $employee_name  =   ! empty( $emp_sorted[$single->user_id] ) ? $emp_sorted[$single->user_id] : '';
        $list[] = [
            'id'               => $single->id,
...
function erp_asset_get_allotment_status_count() {
    ...
    $statuses = [
        'all'              => __( 'All', 'erp-pro' ),
        'allotted'         => __( 'Alllotted', 'erp-pro' ),
        'returned'         => __( 'Returned', 'erp-pro' ),
        'dissmissed'       => __( 'Dissmissed', 'erp-pro' ),
