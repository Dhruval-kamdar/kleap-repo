...
/**
 * Plugin Name: WP ERP - SMS Notification
 * Description: Send SMS notifications to employees and CRM contacts
 * Plugin URI: http://wperp.com/downloads/sms-notification/
 * Author: weDevs
 * Author URI: http://wedevs.com
 * Version: 1.1.1
 * License: GPL2
 * Text Domain: erp-sms-notification
...
class Module {
    ...
    public function add_settings_page( $settings = [] ) {
        $settings[] = include WPERP_SMS_INCLUDES . '/class-settings-page.php';
        return $settings;
    }
    ...
    public function send( $cell_no_all, $message ) {
        ...
        $this->gateway = new \WeDevs\ERP\SMS\Gateway_Handler();
    ...
    public function crm_feeds_load_sms_js_template_old(){
        ...
        if ( isset( $_GET['section'] ) && ( $_GET['section'] == 'contacts' || $_GET['section']  == 'companies' ) ) {
            erp_get_vue_component_template( WPERP_SMS_VIEWS . '/js-templates/new-sms-note.php', 'erp-crm-new-sms-note-template' );
            erp_get_vue_component_template( WPERP_SMS_VIEWS . '/js-templates/timeline-sms-note.php', 'erp-crm-timeline-feed-sms-note' );
            erp_get_vue_component_template( WPERP_SMS_VIEWS . '/js-templates/timeline-sms-note.php', 'erp-crm-timeline-feed-sms-note' );
        }
     ...
     * @return void
     */
    public function crm_sms_save_sms_feeddata( $postdata ) {
        ...
        if ( $postdata['type'] == 'sms' ) {
            ...
            array_walk( $extra_data['sms_number'], function( &$items ) {
                $items = ltrim( $items, '+' );
            });
