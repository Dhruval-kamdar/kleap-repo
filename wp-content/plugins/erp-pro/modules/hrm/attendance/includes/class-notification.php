...
class Notification {
    ...
    function send_email_notification() {
        ...
        $attendances = \WeDevs\ERP\HRM\Attendance\Models\Attendance::whereDate( 'date', '=', $today )
                                                                   ->select( 'user_id' )
                                                                   ->get()
                                                                   ->toArray();
        $attendee    = wp_list_pluck( $attendances, 'user_id' );
        //find who have already checked in
        $leaves         = \WeDevs\ERP\HRM\Models\Leave_Request::where( 'last_status', '1' )
        ...
        $users = \WeDevs\ERP\HRM\Attendance\Models\Employee::where( 'status', 'active' )
                                                           ->whereNotIn( 'user_id', $users_not_to_notify )
                                                           ->select( 'user_id' )
                                                           ->get()
                                                           ->toArray();
        $emailer = wperp()->emailer->get_email( 'Attendance_Reminder' );

        if ( is_a( $emailer, '\WeDevs\ERP\Email' ) ) {
            foreach ( $users as $user ) {
                $emailer->trigger( new Employee( $user['user_id'] ) );
            }
        }
