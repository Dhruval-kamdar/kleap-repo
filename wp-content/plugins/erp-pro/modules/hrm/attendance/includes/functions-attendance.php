...
function erp_att_get_single_attendance( $args ) {
    ...
    $att_by_date = new \WeDevs\ERP\HRM\Attendance\Queries\AttendanceByDate( $args['date'] );
...
function erp_att_insert_attendance( $shift_id, $present, $checkin, $checkout ) {
	$attendance           = \WeDevs\ERP\HRM\Attendance\Models\Attendance::find( absint( $shift_id ) );
...
function erp_att_is_ip_allowed( $ip_address ) {
	...
	if ( \WeDevs\ERP\HRM\Attendance\IpUtils::checkIp( $ip_address, $allowed_ips ) ) {
		return true;
	}
...
function erp_attendance_insert_shift( $start_time, $end_time, $shift_name = null, $holidays = [], $status = 1 ) {
	...
	if ( $end_time_timestamp >= $date_period_timestamp ) {
		return new WP_Error( 'invalid-shift-range', __( 'Time range for the shift is invalid. Please choose shift length less than 24hrs', 'erp_attendance' ) );
	}
	...
	$exist = $wpdb->get_col( "SELECT id FROM {$wpdb->prefix}erp_attendance_shifts as shifts where (shifts.start_time = '$start_time' AND shifts.end_time = '$end_time') OR  shifts.name ='$shift_name'" );
	if ( $exist ) {
		return new WP_Error( 'duplicate-shift', __( 'A shift with the same name/time range is already exists. Duplicate shift is not allowed.', 'erp_attendance' ) );
	}
	...
	if ( ! $inserted ) {
		return new WP_Error( 'shift-insert-failed', __( 'Could not create shift.', 'erp_attendance' ) );
	}
...
function erp_attendance_update_shift( $shift_id, $start_time, $end_time, $shift_name = null, $holidays = [], $status = 1 ) {
	...
	// $exist = $wpdb->get_col( "SELECT id FROM {$wpdb->prefix}erp_attendance_shifts as shifts where (shifts.start_time = '{$start_time}' AND id <> {$shift_id} AND shifts.end_time = '$end_time') OR shifts.name ='$shift_name'" );

	$exist = $wpdb->get_col( "SELECT id FROM {$wpdb->prefix}erp_attendance_shifts as shifts where shifts.name ='$shift_name' AND id <> {$shift_id}" );
...
function erp_attendance_insert_shifting_for_user( $shift_id, $user_id, $start_date = null, $end_date = null, $overwrite = false ) {
	...
	if ( $overwrite ) {
		$dates_join = join( "','", $valid_dates );
		$wpdb->query( "DELETE FROM {$wpdb->prefix}erp_attendance_date_shift WHERE date in ('$dates_join') AND user_id='$user_id'" );
    }
...
function erp_attendance_assign_shift( $user_id, $shift_id, $overwrite = false ) {
	...
	if ( ! $overwrite && $user_shift ) {
		...
		return new WP_Error( 'invalid-action', __( 'User already assigned to a shift. Do you want to overwrite?', 'erp_attendance' ) );
	}

	if ( $user_shift && ! empty( $user_shift->id ) && $user_shift->id == $shift_id ) {
        $sql = sprintf( "UPDATE {$wpdb->prefix}erp_attendance_shift_user SET status='%s'  WHERE shift_id='%s' AND user_id='%s'",intval(1), intval( $shift_id ), intval( $user_id ) );
        $result = $wpdb->query( $sql );
		return new WP_Error( 'invalid-action', __( 'User already assigned to the shift before.', 'erp_attendance' ) );
	}
...
function erp_att_remove_hr_entry_attendances( $dates ) {
	...
	$wpdb->query( "DELETE FROM {$wpdb->prefix}erp_attendance_date_shift WHERE date IN ({$dates})" );
        ...
        if( $dt['hr_should_change'] == true ) {
            ...
            if ( $get_current_log_count > 0 ){
                ...
                if( ! empty( $hr_checkout ) ) {
                    $time_c = strtotime( $hr_checkout_datetime ) - strtotime( $last_row->checkin );
                    $wpdb->update(
                        "{$wpdb->prefix}erp_attendance_log",
                        array(
