...
class Attendance_Controller extends REST_Controller {
        ...
        /*foreach ( $attendances as $att ) {
            if ( ! empty( $att['checkin'] ) ) {
                ...
                if ( ! empty( $att['checkin_id'] ) ) {
                    update_new_attendance_entry_by_hr( $date, $att['user_id'], $att['dshift_id'], $att['checkin_id'], $att['checkout_id'], $checkin, $checkout );
                } else {
    ...
    public function get_employees( \WP_REST_Request $request ) {
        ...
        foreach ( $items as $item ) {
            ...
            $get_shift = $wpdb->get_row( "SELECT easu.shift_id as shift_id, easu.user_id as user_id, eas.name as shift_name FROM `{$wpdb->prefix}erp_attendance_shift_user` as easu LEFT JOIN {$wpdb->prefix}erp_attendance_shifts as eas ON easu.shift_id = eas.id WHERE easu.user_id = {$item->user_id}" );

            $additional_fields = [
                'shift' => ( isset( $get_shift->shift_name ) ) ? $get_shift->shift_name : '-'
            ];
