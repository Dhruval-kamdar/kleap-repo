        ...
        switch ($column_name) {
            ...
            case 'action':
                $actions = [
                    ...
                    'email'  => sprintf( '<a class="button button-sescondary button-small" href="mailto:%s"><span class="dashicons dashicons-email-alt"></span></a>', $item['email'] )
                ];

                $attach_id = erp_people_get_meta($item['applicant_id'], 'attach_id', true);

                if ( $attach_id ) {
                    $actions['cv'] = sprintf( '<a title="%s" class="button button-sescondary button-small" target="_blank" href="%s"><span class="dashicons dashicons-download"></span></a>', __( 'Download CV', 'erp-pro' ), wp_get_attachment_url( $attach_id ) );
                }

                return implode( $actions, ' ' );
                // return sprintf(__('<a class="button button-sescondary button-small" href="%s"><span class="dashicons dashicons-visibility"></span></a> <a class="button button-sescondary button-small" href="%s"><span class="dashicons dashicons-email-alt"></span></a><div>%s</div>'), $jobseeker_preview_url, $email_action_url, erp_people_get_meta($item['id'], 'ip', true ) );
            default:
        }
