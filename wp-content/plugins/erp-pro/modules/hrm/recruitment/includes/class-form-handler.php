...
class Form_Handler {
        ...
        if ( $action ) {
            switch ($action) {
                ...
                case 'download_all_cv' :
                    ...
                    unlink ( $upload_dir['path'] . '/' . $zipname );
                    $zip = new \ZipArchive();
                    $zip->open( $upload_dir['path'] . '/' . $zipname,  \ZipArchive::CREATE );
                    ...
                    foreach ( $applicants_ids as $aids ) {
                        foreach ( $aids as $key => $value ) {
                            $attach_id = erp_people_get_meta( $aids['applicant_id'], 'attach_id', true );
                    ...
                    header( "Content-Type: application/force-download" );
                    readfile( $upload_dir['path'] . '/' . "$zipname" );
                    exit();
            }
        }
    }
    ...
    public function create_opening() {
        ...
        $location = add_query_arg( array( 'action' => 'edit', 'step' => 'hiring_workflow', 'postid' => $postid ), erp_rec_url( 'add-opening' ) );
    ...
    public function add_hiring_workflow() {
        ...
        $location = add_query_arg( array( 'action' => 'edit', 'step' => 'job_information', 'postid' => $jobid ), erp_rec_url( 'add-opening' ) );
        wp_redirect( $location );
    }
    ...
    public function add_job_information() {
        ...
        $location = add_query_arg( array( 'action' => 'edit', 'step' => 'candidate_basic_information', 'postid' => $postid ), erp_rec_url( 'add-opening' ) );
        wp_redirect( $location );
    }
    ...
    public function add_candidate_basic_information() {
        ...
        $location = add_query_arg( array( 'action' => 'edit', 'step' => 'questionnaire', 'postid' => $postid ), erp_rec_url( 'add-opening' ) );
        wp_redirect( $location );
    }
    ...
    public function make_employee() {
        ...
        if ( isset( $_POST['submit'] ) ) {
            ...
            if ( !isset( $first_name ) ) {
                ...
                $this->send_error('User email is empty!');
            } else {
                ...
                $page = ( version_compare( WPERP_VERSION, '1.4.0', '>=' ) ) ? 'erp-hr&section=employee' : 'erp-hr-employee';
