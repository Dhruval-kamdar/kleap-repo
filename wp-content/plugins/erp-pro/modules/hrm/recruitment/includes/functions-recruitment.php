...
function erp_rec_get_recruitment_minimum_experience() {
    $min_exp = array(
        ...
        '1 - 3 Year'     => __( '1 - 3 Year', 'erp-pro' ),
        '3 - 5 Years'    => __( '3 - 5 Years', 'erp-pro' ),
        '5 - 7 Years'    => __( '5 - 7 Years', 'erp-pro' ),
        '7 - 10 Years'   => __( '7 - 10 Years', 'erp-pro' ),
...
function send_email_after_applied_new_job( $data ) {
    ...
    $applicant_id   = ( isset( $data['job_id'] ) ) ? $data['applicant_id'] : 0;
    ...
    $sql = "SELECT
              ...
              people.email,
              a.apply_date
             FROM {$wpdb->prefix}erp_application as a
             LEFT JOIN {$wpdb->prefix}posts as post
              ON post.ID = a.job_id
             LEFT JOIN {$wpdb->prefix}erp_peoples as people
              ON people.id = a.applicant_id
             WHERE a.job_id = {$job_id} AND a.applicant_id = {$applicant_id}";
...
function send_new_job_application_email_to_hr( $data ) {
    ...
    $emailer           = wperp()->emailer->get_email( 'New_Job_Appication_Submitted' );
...
function send_confirmation_of_submission_email_to_applicant( $data ) {
    ...
    $emailer           = wperp()->emailer->get_email( 'Confirmation_Of_Successful_Submission' );
    if ( is_a( $emailer, '\WeDevs\ERP\Email' ) ) {
        $emailer->trigger( $data );
    }
}
