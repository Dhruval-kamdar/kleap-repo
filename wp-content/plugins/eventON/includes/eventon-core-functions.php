...
/* 2.2.17 */
	// process taxnomy filter values and return terms and operator -- DEP v2.8
		...
		if($repeat_type=='custom'&& !empty($_POST['repeat_intervals'])){
			...
			if($numberof_repeats>0){
				...
				foreach($_post_repeat_intervals as $field => $interval){
					...
					if(isset($interval['type']) && isset($interval['type'])=='dates'){
						...
						// start time
						$__ti = ($_is_24h)?
							date_parse_from_format($def_date_format.' H:i', $interval[0]):
							date_parse_from_format($def_date_format.' g:ia', $interval[0]);

						// end time
						$__tie = ($_is_24h)?
							date_parse_from_format($def_date_format.' H:i', $interval[1]):
							date_parse_from_format($def_date_format.' g:ia', $interval[1]);
			...
			for($x =0; $x<=$repeat_count; $x++){
				...
				if($repeat_type == 'monthly' && $month_repeat_by=='dow' && !empty($days) && is_array($days) ){
					...
					$repeat_multiplier = ((int)$repeat_gap) * $x;
					if($term) $new_unix_S = strtotime('+'.$repeat_multiplier.' '.$term, $unix_S);
					if($term) $new_unix_E = strtotime('+'.$repeat_multiplier.' '.$term, $unix_E);
					// add new intervals to array
...
// @version 2.4.7
	function evo_get_term_meta($tax, $termid, $options='', $secondarycheck= false){
		$termmetas = !empty($options)? $options: EVO()->calendar->tax_meta;
