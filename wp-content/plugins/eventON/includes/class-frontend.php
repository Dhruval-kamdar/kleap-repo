...
class evo_frontend {
	...
	public function __construct(){
		...
		$load_only_on_evo_pages = evo_settings_val('evo_load_scripts_only_onevo', $this->evo_options);

		if(!$load_only_on_evo_pages){
			add_action( 'wp_enqueue_scripts', array( $this, 'load_evo_scripts_styles' ), 10 );
			add_action( 'wp_enqueue_scripts', array( $this, 'load_dynamic_evo_styles' ), 50 );
		}
		if($load_only_on_evo_pages && 	evo_settings_val('evo_load_all_styles_onpages', $this->evo_options)
		){
