	...
	// TAXONOMY - event location
			...
			function eventon_taxonomy_add_new_meta_field() {
				...
				// additional fields
					foreach($this->get_event_tax_fields_array('event_location') as $field=>$value){
						if(in_array($field, array('term_name','description','location', 'evcal_lat','evcal_lon','evcal_location_link','evo_loc_img','submit','location_getdir_latlng' ))) continue;
		...
		// Edit term page
			function eventon_taxonomy_edit_meta_field($term) {
				...
				<tr class="form-field">
					<th scope="row" valign="top"><label for="term_meta[location_getdir_latlng]"><?php _e('Use Lat/Lng for get directions location','eventon'); ?></label></th>
					<td>
						<p><span class='yesno_row evo'>
							<?php 	
