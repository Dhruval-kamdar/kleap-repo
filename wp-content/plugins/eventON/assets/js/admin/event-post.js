...
jQuery(document).ready(function($){
	...
	// repeating events UI	
		...
		$('body').on('click','span.evo_repeat_type_val',function(){
			...
			O.addClass('select');
			O.siblings('input').val( O.html() );


			var field = O.data('val');
			...
			$('#evo_add_repeat_interval').on('click',function(){
				...
				if(!obj.is(':visible')){
					...
					if( obj.find('.ristD').val() &&
						...
						var html = '<li data-cnt="'+count+'" class="new"><span>from</span>'+obj.find('.ristD').val()+' '+obj.find('.ristT').val()+' <span class="e">End</span> '+obj.find('.rietD').val()+' '+obj.find('.rietT').val()+'<em alt="Delete">x</em>';
						html += '<input type="hidden" name="repeat_intervals['+count+'][0]" value="'+obj.find('.ristD_h').val()+' '+obj.find('.ristT').val()+'"/>';
						html +='<input type="hidden" name="repeat_intervals['+count+'][1]" value="'+obj.find('.rietD_h').val()+' '+obj.find('.rietT').val()+'"/>';
	...
	//date picker on		
		$('#evo_dp_from').datepicker({ 
			dateFormat: date_format,
			numberOfMonths: 2,
			altField: $('input._real_event_start'),
			altFormat: 'yy/mm/dd',
			onSelect: function( selectedDate , obj) {
				var date = $(this).datepicker('getDate');

				// update end time
				$( "#evo_dp_to" ).datepicker( "setDate", date );
				$( "#evo_dp_to" ).datepicker( "option", "minDate", date );
		...
		$('#evo_dp_to').datepicker({ 
			dateFormat: date_format,
			numberOfMonths: 2,
			altField: $('input._real_event_end'),
			altFormat: 'yy/mm/dd',
