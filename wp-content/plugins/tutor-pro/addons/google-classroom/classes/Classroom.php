...
class Classroom{
    ...
    function __construct($owner_id=null, $post_id=null, $ret=false){
        ...
        $this->credential_owner = $owner_id ?? ($post_id ? get_post_field('post_author', $post_id) : get_current_user_id());
    ...
    private function get_credential_path($serial=null){
        return $this->upload_dir . DIRECTORY_SEPARATOR . 'credential-' . ($serial ?? $this->current_credential) . '.json';
    }

    private function get_token_path($serial=null){
        return $this->upload_dir . DIRECTORY_SEPARATOR . 'token-' . ($serial ?? $this->current_credential) . '.json';
    }
    ...
    private function filter_privileged_content($remote_id, $contents){
        ...
        foreach($contents as $content){
            ...
            $specific_students = $content->individualStudentsOptions->studentIds ?? array();
    ...
    public function get_all_remote_attachments($local_id){
        ...
        foreach($course_works as $work){
            $materials=array_merge($materials, ($work->materials ?? []));
        }     
                   
        // Get stream materials
        $streams = $this->get_stream($local_id, null, true);
        foreach($streams as $stream){
            $materials=array_merge($materials, ($stream->materials ?? []));
        }     
    ...
    private function import_course_post($course){
        ...
        if($existing_id){
            ...
            $post_name = sanitize_title($course->descriptionHeading ?? '');
    ...
    private function get_unique_value(string $table, string $column, $value, $index=1){
        ...
        $duplicate = $wpdb->get_results('SELECT '.$column.' FROM '.$table_name.' WHERE '.$column.'="'.$wpdb->escape($compare_value).'"');
