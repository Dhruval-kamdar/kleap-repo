(function ($) {
	...
	var wcf_flow_analytics = function () {
		...
		$(document).ready(function () {
			var parent = $('.wcf-flow-enable-analytics'),
				analytics_checkbox = $('.wcf-flow-enable-analytics input[type="checkbox"]');
			wcf_show_hide_flow_analytics(parent, analytics_checkbox);
		});

		$('.wcf-flow-enable-analytics input[type="checkbox"]').on('click', function () {
			var analytics_checkbox = $(this),
				parent = analytics_checkbox.closest('.wcf-flow-enable-analytics');
			wcf_show_hide_flow_analytics(parent, analytics_checkbox);
		});

		var wcf_show_hide_flow_analytics = function (parent, analytics_checkbox) {
			...
			if (confirm(cartflows_admin.confirm_msg_for_analytics)) {
				...
				$.ajax({
					...
					success: function (response) {
						if (response.success) {
							alert(cartflows_admin.succesful_msg_for_analytics);
						}
					}
				});
			}
	...
	var wcf_ab_test_events = function () {
		...
		$('.wcf-flow-settings .wcf-step-abtest').on('click', function (e) {
			...
			var delete_status = confirm("This action will enable split test for this step. Are you sure?");
		...
		$('.wcf-flow-settings .wcf-start-split-test').on('click', function (e) {
			...
			$.ajax({
				...
				type: 'POST',
				success: function (data) {
					$this.removeClass('wcf-start-ab-test wcf-stop-ab-test')
					if (data.start) {
		...
		$('.wcf-flow-settings .wcf-ab-test-step-delete').on('click', function (e) {
			...
			var delete_status = confirm("Do you want to delete this variation and its data? Are you sure?");
			...
			if (true == delete_status) {
				...
				$.ajax({
					...
					data: {
						...
						post_id: post_id,
						step_id: step_id,
						security: cartflows_admin.wcf_delete_ab_test_step_nonce
					},
					dataType: 'json',
					type: 'POST',
					success: function (data) {
		...
		$('.wcf-flow-settings .wcf-ab-test-step-archive').on('click', function (e) {
			...
			var delete_status = confirm("Do you want to archive this variation? Are you sure?");

			if (true == delete_status) {

				console.log('Archiving Step ');
				icon_span.addClass('wp-ui-text-notification');
				text_span.addClass('wp-ui-text-notification').text('Archiving...');
				...
				$.ajax({
					...
					data: {
						...
						post_id: post_id,
						step_id: step_id,
						security: cartflows_admin.wcf_archive_ab_test_step_nonce
					},
					dataType: 'json',
					type: 'POST',
					success: function (data) {
		...
		$('.wcf-flow-settings .wcf-ab-test-step-clone').on('click', function (e) {
			...
			var delete_status = confirm("Do you want to clone this variation? Are you sure?");
			...
			if (true == delete_status) {
				...
				$.ajax({
					...
					data: {
						...
						security: cartflows_admin.wcf_clone_ab_test_step_nonce
					},
					dataType: 'json',
					type: 'POST',
					success: function (data) {
		...
		$('.wcf-flow-settings .wcf-declare-winner').on('click', function (e) {
			...
			var delete_status = confirm("Do you want to declare this variation a winner and archive the other variations? Are you sure? ");
			...
			if (true == delete_status) {
				...
				$.ajax({
					...
					data: {
						...
						flow_id: post_id,
						step_id: step_id,
						security: cartflows_admin.wcf_declare_ab_test_winner_nonce
					},
					dataType: 'json',
					type: 'POST',
					success: function (data) {
		...
		$('.wcf-flow-settings .wcf-step-archive-restore').on('click', function (e) {
			...
			var restore_status = confirm("Do you want to restore this archived variation? Are you sure?");
			...
			if (true == restore_status) {
				...
				$.ajax({
					...
					data: {
						...
						step_id: step_id,
						control_id: control_id,
						security: cartflows_admin.wcf_restore_archive_ab_test_variation_nonce
					},
					dataType: 'json',
					type: 'POST',
					success: function (data) {
		...
		$('.wcf-flow-settings .wcf-step-archive-delete').on('click', function (e) {
			...
			var delete_status = confirm("Do you want to delete this archived variation? Are you sure?");
			...
			if (delete_status) {
				...
				$.ajax({
					url: ajaxurl,
					data: {
						action: "cartflows_delete_archive_ab_test_variation",
						flow_id: post_id,
						step_id: step_id,
						control_id: control_id,
						security: cartflows_admin.wcf_delete_archive_ab_test_variation_nonce
					},
					dataType: 'json',
					type: 'POST',
					success: function (data) {
		...
		$(document).on('click', '.wcf-analytics-table .wcf-ab-test-row', function (e) {
			...
			var $this = $(this),
				...
				show_rows = parent.find('.wcf-ab-test-inner-row[data-parent-id="' + step_id + '"]');

			icon_span.toggleClass("dashicons-arrow-right-alt2 dashicons-arrow-down-alt2");
			show_rows.toggle();
		});
		...
		$(document).on("click", "#wcf-archived-button", function (e) {
			...
			$this.find('i').toggleClass('dashicons-arrow-right  dashicons-arrow-down');
		...
		var ab_settings_all_events = function () {
			...
			var close_ab_settings_popup = function (current_selector) {
				current_selector.closest('.wcf-ab-test-settings-overlay').hide();
			};

			/* Ab test settings */
			var show_close_ab_settings = function () {

				$('.wcf-flow-settings .wcf-settings-split-test').on('click', function (e) {

					e.preventDefault();

					var $this = $(this),
						parent = $this.parents('.wcf-ab-test-head'),
						current_content_wrap_html = parent.find('.wcf-ab-test-content-wrap').html(),
						settings_overlay = $('.wcf-ab-test-settings-overlay');

					settings_overlay.find('.wcf-content-wrap').html(current_content_wrap_html);
				...
				$('.wcf-ab-test-settings-overlay').on('click', '.wcf-popup-close-wrap, .wcf-ab-test-cancel', function (e) {
				...
				$(document).on('click', '.wcf-ab-test-settings-overlay', function (e) {
					if ($(event.target).hasClass('wcf-ab-test-settings-overlay')) {
			...
			var init_ab_traffic_fields = function (wrapper) {
				...
				slider.each(function () {
					...
					range.on('input', function (event) {
						...
						let current_this = $(this),
							settings_parent = current_this.closest('.wcf-ab-settings-content'),
							range_parent = current_this.closest('.wcf-traffic-slider-wrap'),
							current_value = parseInt(this.value),
							current_variation_id = range_parent.data('variation-id'),
							prev_diff = variations[current_variation_id] - current_value;
						...
						range_input.val(current_value).attr('value', current_value);

						/* Update other variations traffic */
						update_variations_traffic(settings_parent, current_variation_id, prev_diff);
					});
					...
					range_input.on('input', function (event) {
						...
						/* Update value and attr */
						current_this.attr('value', current_value);
						range.val(current_value).attr('value', current_value);
				...
				var update_variations_traffic = function (parent, changed_variation_id, diff) {

					$.each(variations, function (variation_id, traffic) {

						//Ignores the selected variation
						if (parseInt(variation_id) === changed_variation_id) {
							return true;
						}
						...
						parent.find('.wcf-traffic-range-' + variation_id + ' input').val(new_value).attr('value', new_value);
						parent.find('.wcf-traffic-value-' + variation_id + ' input').val(new_value).attr('value', new_value);
					});
				}
			};
			...
			var save_ab_settings = function () {
				...
				$('.wcf-ab-test-settings-overlay .wcf-ab-test-save').on('click', function (e) {

					e.preventDefault();

					var $this = $(this),
						old_text = $this.text(),
						parent = $this.closest('.wcf-ab-test-settings-overlay'),
						content_wrap_html = parent.find('.wcf-content-wrap').html(),
						post_id = $('form#post #post_ID').val(),
						step_id = parent.find('.wcf-ab-settings-content').attr('data-id'),
						form_data = decodeURIComponent(parent.find('input[name], select[name], textarea[name]').serialize());
					...
					$.ajax({
						...
						data: {
							...
							step_id: step_id,
							form_data: form_data,
							security: cartflows_admin.wcf_save_ab_test_settings_nonce
						},
						dataType: 'json',
						type: 'POST',
						success: function (data) {

							$('.wcf-content-wrap-' + step_id).html(content_wrap_html);
