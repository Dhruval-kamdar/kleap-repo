(function($){
	...
	var wcf_product_variation_fields_events = function() {
		...
		$('.wcf-column-right .wcf-product-options .wcf-pv-checkboxes .field-wcf-enable-product-options input:checkbox').on('change', function(e) {
			wcf_product_variation_fields();
		});

		$('.wcf-column-right .wcf-product-options .wcf-pv-fields .field-wcf-enable-product-variation input:checkbox').on('change', function(e) {
			wcf_product_variation_fields();
		});
	...
	var wcf_product_variation_fields = function() {
		...
		var wrap 			= $('.wcf-checkout-table'),
			custom_fields 	= wrap.find('.wcf-column-right .wcf-product-options .wcf-pv-checkboxes .field-wcf-enable-product-options input:checkbox');
		...
		var pv_options_chkbox = wrap.find('.wcf-column-right .wcf-product-options .wcf-pv-fields .field-wcf-enable-product-variation input:checkbox');
	...
	var wcf_offer_product_variation_field_events = function() {
		...
		var wcf_offer_product_variation_fields = function() {
			...
			var wrap 			= $('.wcf-offer-table'),
				pv_options_chkbox = wrap.find('.wcf-column-right .wcf-offer-general .field-wcf-enable-offer-product-variation input:checkbox');
			
			var pv_options = [
				'.field-wcf-offer-product-variation-options',
			];
		...
		/* Change product variation Fields*/
		$('.wcf-column-right .wcf-offer-general .field-wcf-enable-offer-product-variation input:checkbox').on('change', function(e) {
			wcf_offer_product_variation_fields();
		});
	};
	...
	var wcf_hide_highlight_field= function(){
		...
		$(document).on('ready',function (e) {
			wrap = $('.wcf-product-option-fields');
			wrap.find('.wcf-field-enable-highlight-option-field input').each(function(e){
				$this = $(this);
		...
		$(document).on( 'change', '.wcf-field-enable-highlight-option-field input', function(e) {
			 $this = $(this);
			 parent = $this.closest('.wcf-product-option-fields');
			 show_hide_highlight_field($this,parent);
		});
	...
	var wcf_toggle_product_options = function(){
	  		...
	  		$(document.body).on('click','.wcf-product-field-item-bar',function(e){
	  			$this = $(this);
	...
	var wcf_sync_products = function(){
		...
		$(document.body).on('click','[data-tab=wcf-product-options]',function(){

			var product_options_obj = []; 
			
			$('.wcf-product-option-fields .wcf-product-field-item-settings').each(function (e) {

				var $this = $(this),
					id = $this.parent().attr('data-product-id'),
					title = $this.find('.wcf-field-product-title-field input').val(),
					subtext = $this.find('.wcf-field-subtext-field input').val(),
					highlight = $this.find('.wcf-field-enable-highlight-option-field input[type="checkbox"]'),
					highlight_text = $this.find('.wcf-field-highlight-text-field input').val(),
					po_unique_id = $this.find('.wcf-product-options-unique-id').val();
			...
			$('.wcf-repeatable-row').each(function (e) {
				...
				var repeatable_row = $(this),
					product_input = repeatable_row.find( 'select.wcf-product-search' ),
					product_id = product_input.val(),
					name = product_input.find('option:selected').text(),
					template = $('#tmpl-wcf-product-option-repeater').html(),
					unique_id = repeatable_row.find('.wcf-repeatable-row-unique-id-field input[type="hidden"]').val();

				template = template.replace( /{{original_product_id}}/g,product_id );
				template = template.replace( /{{original_product_name}}/g,name );
				...
				if( unique_id in product_options_obj ) {
					input_product_name 	 = product_options_obj[ unique_id ][ 'title' ];
					input_subtext 		 = product_options_obj[ unique_id ][ 'subtext' ];
					input_enable_highlight = product_options_obj[ unique_id ][ 'enable_highlight' ];
					input_highlight_text 	 = product_options_obj[ unique_id ][ 'highlight_text' ];
				}
				
				template = template.replace( /{{input_product_name}}/g, input_product_name );
				template = template.replace( /{{input_subtext}}/g, input_subtext );
				template = template.replace( /{{input_highlight_text}}/g,input_highlight_text );
				
				var template_node = $(template);

				if('yes' == input_enable_highlight){
					template_node.find('.wcf-field-enable-highlight-option-field input[type="checkbox"]').attr("checked","checked");
				}else{
					template_node.find('.wcf-field-highlight-text-field').hide();
				}
