	...
	public function product_price() {
		...
		if ( _is_wcf_base_offer_type() ) {
			...
			if ( $offer_product['original_price'] !== $offer_product['unit_price'] ) {
				$output .= '<span class="wcf-regular-price del">' . wc_price( $offer_product['original_price'] ) . '</span>';
				$output .= '<span class="wcf-discount-price">' . wc_price( $offer_product['unit_price'] ) . '</span>';
			} else {
				if ( in_array( $product->get_type(), array( 'subscription', 'variable_subscription', 'subscription_variation' ), true ) ) {
					$output .= '<span class="wcf-regular-price">' . WC_Subscriptions_Product::get_price_string( $product ) . '</span>';
				} else {
					$output .= '<span class="wcf-regular-price">' . wc_price( $offer_product['unit_price'] ) . '</span>';
				}
			}

			if ( 0 < $offer_product['shipping_fee'] ) {
				$output .= '<span class="wcf-offer-shipping-fee">' . __( 'Shipping: ', 'cartflows-pro' );
				$output .= wc_price( $offer_product['shipping_fee'] );
