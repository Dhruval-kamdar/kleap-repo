...
class Cartflows_Pro_Analytics_Reports {
	...
	public function reset_analytics_button() {
		...
		$reset_btn_markup         .= "<div class='wcf-reset-button-wrap'>";
			$reset_btn_markup     .= "<button class='wcf-reset-analytics button button-secondary' id='wcf-reset-analytics-button'>";
	...
	public function fetch_visits( $flow_id ) {
		...
		$query = $wpdb->prepare(
			"SELECT step_id,
			 ...
			 FROM $visit_db LEFT JOIN $visit_meta_db ON $visit_db.id = $visit_meta_db.visit_id 
			 WHERE step_id IN ( $step_ids ) 
			 AND ( date_visited BETWEEN %s AND %s ) 
			 GROUP BY step_id",//phpcs:ignore
			$start_date,
			$end_date
		);
		// phpcs:enable
