...
class Cartflows_Pro_Order_Bump_Product {
	...
	public function bump_order() {
		...
		if ( _is_wcf_checkout_type() ) {
			...
			$bump_order_image = Cartflows_Helper::get_image_url( $post->ID, 'wcf-order-bump-image', 'medium' );
	...
	public function order_bump_process() {
		...
		if ( null != $found_item_key && 'true' === $checked ) {
			...
			if ( $discount_enabled && 'coupon' !== $discount_type ) {
				...
				if ( $discount_enabled && 'coupon' === $discount_type ) {
					$apply_coupon = wcf_pro()->utils->apply_discount_coupon( $discount_type, $discount_coupon );
				}
