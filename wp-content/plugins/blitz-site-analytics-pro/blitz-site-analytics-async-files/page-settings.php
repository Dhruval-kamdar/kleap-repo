<div class="wrap">
	<h2><?php _e( 'Site Analytics', $this->text_domain ) ?></h2>
	...
	?>
		...
		<h3 class="title ga-basic-tracking"><?php _e( 'Basic Network Tracking Settings', $this->text_domain ) ?></h3>
			...
			?>
				...
				<div data-module="api_project" class="sub-options">
					<div class="postbox">
						...
						<button type="submit" name="gaplus_access_by_api" value="<?php echo wp_create_nonce('gaplus_access_by_api'); ?>"  class="button button-primary"><?php _e( 'Authorize', $this->text_domain ) ?></button>
					</div>
				</div>
				<div data-module="code" class="sub-options">
					<div class="postbox">
						<ol>
							<li><?php printf(__( 'Login and get access code <a target="_blank" href="%s">here</a>.', $this->text_domain ), $blitz_site_analytics_async_dashboard->google_client->createAuthUrl() ); ?></li>
							<li><?php _e( 'Input access code: ', $this->text_domain ) ?> <input name="code" type="text"/></li>
						</ol>
						<button type="submit" name="gaplus_access_by_code" value="<?php echo wp_create_nonce('gaplus_access_by_code'); ?>" class="button button-primary"><?php _e( 'Authorize', $this->text_domain ) ?></button>
					</div>
				</div>

				<p><?php echo apply_filters('ga_login_method_description', __( 'Access code is quicker solution but setting up Google API project is more suitable for high traffic website and will result in smoother experience for site admins (whole login process will be automatic, no access code copy pasting). You can read more about setting Google API project <a target="_blank" href="https://waas-pro.com/site-analytics-pro/">here</a>')); ?></p>
			<?php
			}
			else {
				echo '<p><a href="'.esc_url(wp_nonce_url(add_query_arg('google_logout', 1), 'gaplus_logout', 'gaplus_logout')).'" class="button button-secondary">'.__( 'Logout from Google account', $this->text_domain ).'</a></p>';
			}
			?>
			</form>
			...
			<form method="post" action="">
				<table  class="form-table ga-basic-tracking">
					<tr class="ga-tracking-code" valign="top">
						...
						<td>
							...
							<p class="description"><?php _e( 'Your Site Analytics tracking code. Ex: UA-XXXXX-X. The Network-wide tracking code will track your entire network of sub-sites.', $this->text_domain ); ?></p>
						</td>
					</tr>
					<tr class="ga-stats" valign="top">
						<th scope="row"><?php _e( 'Site Analytics inside WordPress Dashboard For All Sites In Network', $this->text_domain ); ?></th>
						<td>
							<?php
							if(isset($accounts) ) {
								if(is_array($accounts) && count($accounts) > 0) {
									...
									echo '</select>';
									echo '<p class="description">'.__( 'Choose correct Site Analytics profile to use for displaying statistics inside WordPress admin panel. Please make sure that this is a profile used for network wide tracking.', $this->text_domain ).'</p>';
								}
								else {
									if(isset($blitz_site_analytics_async_dashboard->error))
										echo '<p class="description">'.$blitz_site_analytics_async_dashboard->error.'</p>';
									else
										echo '<p class="description">'.__( 'You do not have any Site Analytics profiles to choose from. Please <a href="http://www.google.com/analytics/">create</a> new one.', $this->text_domain ).'</p>';
								}
							}
							else {
					...
					<tr class="ga-minimum-role-stats" valign="top">
						...
						<td>
							...
							<?php $current = isset($this->current_settings['track_settings']['minimum_capability_reports']) ? $this->current_settings['track_settings']['minimum_capability_reports'] : ''; ?>
								...
								<p class="description"><?php _e( 'Choose minimum role or set custom capability (leave blank to disable) required to see Site Analytics  inside WordPress Dashboard.', $this->text_domain ); ?> <?php _e( 'You can also allow admistrators to overwrite this setting for the sites they control.', $this->text_domain ); ?></p>
						</td>
					</tr>

					<?php
					if ( function_exists('is_pro_site') ):
						$levels = get_site_option( 'psts_levels', false );
					if ( is_array( $levels ) && $levels ):
					?>
						<tr class="ga-minimum-pro-features" valign="top">
						<th scope="row">
							<?php _e( 'Minimum Pro Site level for use of Site Analytics features', $this->text_domain ); ?>
						</th>
						<td>
							<select name="supporter_only">
							 ...
							 <p class="description"><?php _e( 'Enable use of Site Analytics features for sites with selected minimum PRO Site level.', $this->text_domain ); ?></p>
						</td>
					</tr>
						<tr class="ga-minimum-pro-stats" valign="top">
						<th scope="row">
							<?php _e( 'Minimum PRO Site level for Site Analytics  inside WordPress Dashboard', $this->text_domain ); ?>
						</th>
						<td>
							...
							<p class="description"><?php _e( 'Enable network-wide tracking code based Site Analytics  inside WordPress Dashboard for sites with selected minimum PRO Site level.', $this->text_domain ); ?></p>
						</td>
					</tr>
					<?php
					endif;
				...
				<div class="bl-site-analytics-color-settings">
				<h3 class="title ga-color-setting"><?php _e( 'Graph Color Settings', $this->text_domain ) ?></h3>
				<table>
				<tr>
					<th>Pageview</th>
					<td><input type="text" id="ga-pageview-color" class="ga-color-field pageview-color" name="ga_pageview_color" value="<?php if ( !empty( $this->current_settings['track_settings']['ga_pageview_color'] ) ) { echo $this->current_settings['track_settings']['ga_pageview_color']; } ?>" data-default-color="#2F5EC4"></td>
					</tr>
				<tr>
					<th>Visits</th>
				    <td><input type="text" id="ga-visitors-color" class="ga-color-field visitors-color" name="ga_visits_color" value="<?php if ( !empty( $this->current_settings['track_settings']['ga_visits_color'] ) ) { echo $this->current_settings['track_settings']['ga_visits_color']; } ?>" data-default-color="#ED750A"></td>
				    </tr>
				<tr>
					<th>Unique Visitors</th>
					<td><input type="text" id="ga-unique-color" class="ga-color-field unique-color" name="ga_univisits_color" value="<?php if ( !empty( $this->current_settings['track_settings']['ga_univisits_color'] ) ) { echo $this->current_settings['track_settings']['ga_univisits_color']; } ?>" data-default-color="#FFC517"></td>
					</tr>
				</table>
				</div>
				<script>
(function( $ ) {
	...
	$(function() {
	var pageview=	$("#ga-pageview-color").val();
	$('.ga-color-field').wpColorPicker();
	console.log(pageview);
	
	});
	$('.color_btn').click(function(){
    $('.pageview-color').change();
});

$('.pageview-color').change(function(){
	var pageview=	$("#ga-pageview-color").val();
   console.log(pageview);
});
})( jQuery );
				...
				<p class="submit">
					<?php wp_nonce_field('bl_submit_settings_network'); ?>
	...
	?>
		<div id="ga-site-settings">
			<?php
			$initial_description = '
			<p>'.__( 'Site Analytics is the enterprise-class web analytics solution that gives you rich insights into your website traffic and marketing effectiveness. Powerful, flexible and easy-to-use features now let you see and analyze your traffic data in an entirely new way. With Site Analytics, you\'re more prepared to write better-targeted ads, strengthen your marketing initiatives and create higher converting websites.', $this->text_domain ).'</p>
			...
			?>
				...
				<?php if(isset($google_api['verified']) && $google_api['verified'] == true ) { ?>
					<p><a href="<?php echo esc_url(wp_nonce_url('', 'gaplus_login', 'gaplus_login'));?>" class="button button-secondary"><?php _e( 'Login with google account', $this->text_domain ); ?></a></p>
				<?php } else { ?>
			...
			<form method="post" action="">
				<table class="form-table ga-basic-tracking">
					<tr class="ga-tracking-code" valign="top">
						...
						<td>
							...
							<br />
							<p class="description"><?php _e( 'Your Site Analytics tracking code. Ex: UA-XXXXX-X. The Site tracking code will track this site. For more information on finding the tracking code, please visit <a href="https://support.google.com/analytics/answer/1032385?rd=1">this</a> site.', $this->text_domain ); ?></p>
						</td>
					</tr>
					<tr class="ga-stats" valign="top" id="bl-site-analytics-reports-settings">
						<th scope="row"><?php _e( 'Custom Site Analytics  inside WordPress Dashboard', $this->text_domain ); ?></th>
						<td>
							<?php
							...
							if(isset($accounts) ) {
									...
									echo '</select>';
									echo '<p class="description">'.__( 'Choose your own Site Analytics profile to use for displaying statistics inside WordPress admin panel.', $this->text_domain ).'</p>';
								}
								else {
									echo '<p class="description">'.__( 'You do not have any Site Analytics profiles to choose from. Please <a href="http://www.google.com/analytics/">create</a> new one.', $this->text_domain ).'</p>';
								}
				...
				<div id="bl-site-analytics-advanced-settings">
					<h3 class="title ga-advanced-tracking"><?php _e( 'Advanced Site Tracking Settings', $this->text_domain ) ?></h3>
					<table class="form-table ga-advanced-tracking">
						<?php if(!($this->network_settings['track_settings']['anonymize_ip'] && $this->network_settings['track_settings']['anonymize_ip_force'])) { ?>
						<tr class="ga-anonymize-ip" valign="top">
							<th scope="row"><?php _e( 'IP Anonymization', $this->text_domain ); ?></th>
							<td>
								<label><input type="radio" name="anonymize_ip" value="1" <?php if ( !empty( $this->current_settings['track_settings']['anonymize_ip'] ) ) echo 'checked="checked"'; ?> /> <?php _e( 'Enable', $this->text_domain ) ?></label>
						...
						?>
							<tr class="ga-minimum-role-stats" valign="top">
								...
								<td>
									...
									<p class="description"><?php _e( 'Choose minimum role or set custom capability (leave blank to disable) required to see Site Analytics  inside WordPress Dashboard.', $this->text_domain ); ?></p>
								</td>
							</tr>
						<?php
						}
...
(function( $ ) {
	...
	$('.ga-color-field').wpColorPicker();
				...
				<p class="submit">
					<?php wp_nonce_field('bl_submit_settings'); ?>
					<input type="submit" name="submit" class="button-primary" value="<?php _e( 'Save Changes', $this->text_domain ); ?>" />
				</p>
			</form>
		</div>
