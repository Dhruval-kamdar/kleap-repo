<?php
/*  Copyright WaaS-Pro.com */
class Blitz_Site_Analytics_Async_Dashboard {
    var $text_domain;
    var $plugin_url;
    ...
    function __construct() {
        ...
        $this->text_domain = $blitz_site_analytics_async->text_domain;
        $this->plugin_url = $blitz_site_analytics_async->plugin_url;

        include_once( 'helpers-blitz-site-analytics-async-dashboard.php' );

        //required capability
	    $track_settings = bl_ga_plus_get_track_settings();
        ...
        //setup correct Site analytics data source
        $this->is_network_admin = (is_network_admin() || (wp_doing_ajax() && isset($_POST['network_admin']) && $_POST['network_admin'])) ? 1 : 0;

	    $site_settings = bl_ga_plus_get_settings( 'site' );
	    $network_settings = bl_ga_plus_get_settings( 'network' );
	    if (
    ...
    function init_widgets() {
        global $blitz_site_analytics_async;
        if (
            !empty( $blitz_site_analytics_async->network_settings['track_settings']['supporter_only'] )
            && function_exists('is_pro_site')
            && !is_pro_site(get_current_blog_id(), $blitz_site_analytics_async->network_settings['track_settings']['supporter_only'])
        ...
        add_action( 'widgets_init', function () {
            return register_widget("Blitz_Site_Analytics_Async_Frontend_Widget");
        } );
    }
        ...
        //load only for: dashboard, post type page and correct ajax call
         if(
            ...
            (
                ...
                ($pagenow == 'index.php' && isset($_GET['page']) && $_GET['page'] == 'blitz-site-analytics-statistics') ||
    ...
    function admin_enqueue_scripts($hook) {
        ...
        wp_register_script('blitz_site_analytics_async', $this->plugin_url . 'blitz-site-analytics-async-files/ga-async.js', array('jquery','sack', 'google_charts_api'), 330);
        wp_enqueue_script('blitz_site_analytics_async');
        //configure parameters for JS
        $this->setup_script_variables('blitz_site_analytics_async');

        wp_register_style( 'BlitzSiteAnalyticsAsyncStyle', $this->plugin_url . 'blitz-site-analytics-async-files/ga-async.css', array(), 39);
    ...
    function admin_menu() {
        ...
        add_dashboard_page(__('Site Analytics ', $this->text_domain), __('Site Analytics ', $this->text_domain), $this->required_capability, 'blitz-site-analytics-statistics', array( $this, 'google_analytics_statistics_page' ) );
    }
    ...
    function register_bl_site_analytics_dashboard_widget() {
	    $network_settings = bl_ga_plus_get_settings( 'network' );
        if(
            !current_user_can($this->required_capability) ||
            (
        ...
        wp_add_dashboard_widget('google_analytics_dashboard', __('Site Analytics  - Last 30 Days', $this->text_domain), array(&$this, 'google_analytics_widget'));
    }

    function register_bl_site_analytics_post_widget() {
	    $network_settings = bl_ga_plus_get_settings( 'network' );
        if(
            !current_user_can($this->required_capability) ||
            (
        ...
        foreach ( $screens as $screen )
            add_meta_box('google_analytics_dashboard', __('Site Analytics  - Last 30 Days', $this->text_domain), array(&$this, 'google_analytics_widget'), $screen, 'normal');
    }
    ...
    function reauthenticate_notice() {
        ...
        if(current_user_can('manage_options') && isset($site_settings['google_login']['logged_in']) && $site_settings['google_login']['logged_in'] == '1')
            echo '<div class="error"><p>'.sprintf(__('Site Analytics dashboard statistics require reauthentication. You can do it <a href="%s">here</a> and this message will disappear.', $this->text_domain), admin_url('options-general.php?page=blitz-site-analytics')).'</p></div>';

        if(is_super_admin() && isset($network_settings['google_login']['logged_in']) && $network_settings['google_login']['logged_in'] == '1')
            echo '<div class="error"><p>'.sprintf(__('Site Analytics dashboard network statistics require reauthentication. You can do it <a href="%s">here</a> and this message will disappear.', $this->text_domain), network_admin_url('settings.php?page=blitz-site-analytics')).'</p></div>';
    }
    ...
    function settings_page_data() {
        ...
        //load only for: dashboard, post type page and correct ajax call
        if(($pagenow == 'settings.php' || $pagenow == 'options-general.php') && isset($_GET['page']) && $_GET['page'] == 'blitz-site-analytics') {
            //this is just for getting accounts for now and then we need current settings... always.
	        $google_login = bl_ga_plus_get_google_login_settings();
	        if ( bl_ga_plus_is_google_login_logged_in() ) {
                $this->oauth_token = $google_login['token'];
                $this->oauth_secret = $google_login['token_secret'];
            }

            $this->google_login = bl_ga_plus_get_google_login_settings();
    ...
    function init_handle_google_login2() {
        ...
        if(isset($_GET['state'])) {
            ...
            if(isset($state['gaplus_login']) && wp_verify_nonce( $state['gaplus_login'], 'gaplus_login' )) {
                ...
                $url = $url.'?page=blitz-site-analytics&code='.$code.'&gaplus_loggedin='.wp_create_nonce('gaplus_loggedin');
    ...
    function admin_init_handle_google_login2() {
	    ...
	    $google_api = bl_ga_plus_get_google_api_settings( 'network' );
        ...
        if(isset($_POST['gaplus_access_by_api'])) {
            ...
            bl_ga_plus_update_setting( 'google_api', $new_google_api_settings, $is_network );
        ...
        elseif(isset($_GET['gaplus_loggedin']) || (isset($google_api['verified']) && $google_api['verified'] == true )) {
            ...
            $this->google_client->setApplicationName('Site Analytics +');
            $this->google_client->setRedirectUri('urn:ietf:wg:oauth:2.0:oob');
            $this->google_client->setClientId(apply_filters('ga_project_client_id','1086096242072-2bcgqlmbguvt9ip6vvtcntb4pq0ek1p0.apps.googleusercontent.com'));
            $this->google_client->setClientSecret(apply_filters('ga_project_client_secret', '4xIIxZIbTD4R7EApaU-G613h'));
            $this->google_client->setDeveloperKey(apply_filters('ga_project_key', 'AIzaSyARPvqRnAHcdo5OtHg8BNI0eFewn9JgJns'));
        }
        ...
        if(isset($_POST['gaplus_access_by_code']) || isset($_GET['gaplus_loggedin'])) {
            if($_REQUEST['code']) {
                try {
	                $result = bl_ga_plus_code_authenticate( $_REQUEST['code'] );
                    $db_token_id = bl_ga_plus_update_saved_token( $result['google_user_id'], $result['token_object'] );

                    if(isset($_GET['gaplus_loggedin']) && $is_network) {
	                    $google_api = bl_ga_plus_get_google_api_settings( 'network' );
                        $google_api['verified'] = true;
                        bl_ga_plus_update_setting( 'google_api', $google_api, $is_network );
                    }

                    //lets store data for site
	                bl_ga_plus_refresh_google_login_settings( $db_token_id, $is_network );
                	...
                	bl_ga_plus_reset_google_login_settings( $is_network );
                }
            }
        ...
        if( isset($_GET['gaplus_logout']) && wp_verify_nonce($_GET['gaplus_logout'], 'gaplus_logout') ) {
	        ...
	        bl_ga_plus_update_setting( 'google_login', array(), $is_network );
            ...
            wp_redirect(add_query_arg(array('page' => 'blitz-site-analytics', 'dmsg' => urlencode(__( 'Logout successful!', $this->text_domain ))), $redirect_url));
            exit();
        }
        ...
        if(isset($this->google_login['token_id']) && $this->google_login['token_id']) {
            global $wpdb;
            $db_token = bl_ga_plus_get_saved_token_by_token_id( $this->google_login['token_id'] );
            if($db_token) {
        ...
        if(isset($this->google_login['logged_in']) && $this->google_login['logged_in'] == 2) {
	        ...
	        if ( $this->google_login['expire'] < time() ) {
		        ...
		        try {
			        bl_ga_plus_refresh_google_login_token( $this->google_login, $source );
			        $this->google_login = bl_ga_plus_get_google_login_settings();
		        } catch ( GAPGoogle_IO_Exception $e ) {
    ...
    function handle_refesh_token_failure($source, $e) {
	    ...
	    bl_ga_plus_update_setting( 'google_login_failure', $e->getMessage(), $source );
        ...
        if($this->google_login['fail_count'] < 10)
	        bl_ga_plus_update_setting( 'google_login', $this->google_login, $source );
        else
	        bl_ga_plus_update_setting( 'google_login', array(), $source );
    }
        ...
        if(is_wp_error($response)) {
            ...
            if($this->http_code != 200) {
                ...
                foreach($response->items as $analytics_profile) {
                    ...
                    $current_settings = bl_ga_plus_get_settings( 'current' );
                    if(!isset($save_settings) && (empty($current_settings['track_settings']['tracking_code']) || empty($current_settings['track_settings']['google_analytics_account_id']))) {
                        if($current_site_url == $website_url) {
	                        ...
	                        bl_ga_plus_update_setting( 'track_settings', $current_settings['track_settings'], $is_network );
                        }
                    }
    ...
    function set_up_ga_data() {
        ...
        //setup correct Site analytics profile id
        if($this->stats_source == 'site') {
        	$login_settings = bl_ga_plus_get_google_login_settings( 'site' );
        	$track_settings = bl_ga_plus_get_track_settings( 'site' );
        ...
        elseif($this->stats_source == 'network') {
	        $login_settings = bl_ga_plus_get_google_login_settings( 'network' );
	        $track_settings = bl_ga_plus_get_track_settings( 'network' );
            $this->profile_id = $track_settings['google_analytics_account_id'];
        ...
        if($this->profile_id && $this->oauth_token && $this->oauth_secret) {
            ...
            if(is_admin()) {
                ...
                elseif($pagenow == 'index.php' && !isset($_GET['page']))
                    $this->type = 'widget';
                elseif($pagenow == 'index.php' && isset($_GET['page']) && $_GET['page'] == 'blitz-site-analytics-statistics')
            ...
            if($this->type != 'frontend_widget') {
                ...
                add_action('wp_dashboard_setup', array(&$this, 'register_bl_site_analytics_dashboard_widget'));
                add_action('wp_network_dashboard_setup', array(&$this, 'register_bl_site_analytics_dashboard_widget'));
                add_action('add_meta_boxes', array(&$this, 'register_bl_site_analytics_post_widget'));
            }
        }
    }
        ...
        if(!$this->cache) {
            ...
            if($this->error) {
                if (strpos($this->error,'GDataauthErrorAuthorizationInvalid') !== false) {
                    if(is_network_admin())
                        $settings_page_url = admin_url('network/settings.php?page=blitz-site-analytics');
                    else
                        $settings_page_url = admin_url('options-general.php?page=blitz-site-analytics');
    ...
    function google_analytics_widget() {
        ...
        if($this->is_network_admin)
            $statistics_page_url = admin_url('network/index.php?page=blitz-site-analytics-statistics');
        else
            $statistics_page_url = admin_url('index.php?page=blitz-site-analytics-statistics');
        ...
        if(!$this->cache) {
            ...
            echo '<div id="bl-site-analytics-widget">'.$text.'</div><p class="textright"><a class="button button-primary" href="'.$statistics_page_url.'">'.__('See All Stats', $this->text_domain).'</a></p>';
        }
        else
            echo '<input type="hidden" id="ga-pageview-color" class="ga-color-field pageview-color" name="ga_pageview_color" value="'.$current_settings['track_settings']['ga_pageview_color'].'"><input type="hidden" id="ga-visitors-color" class="ga-color-field visitors-color" name="ga_visits_color" value="'.$current_settings['track_settings']['ga_visits_color'].'"><input type="hidden" id="ga-unique-color" class="ga-color-field unique-color" name="ga_univisits_color" value="'.$current_settings['track_settings']['ga_univisits_color'].'"><div id="bl-site-analytics-widget">'.$this->load_google_analytics().'</div><p class="textright"><a class="button button-primary" href="'.$statistics_page_url.'">'.__('See All Stats', $this->text_domain).'</a></p>';
    }

    function google_analytics_widget_html($stats) {
        $return = '
            <div aria-hidden="true" class="google_analytics_chart_holder">
                <div id="bl-site-analytics-chart-visitors" class="google_analytics_chart" style="width: 100%; height: 300px;"></div>
            </div>';
    ...
    function google_analytics_widget_extended_html($stats) {
        $return = '
            ...
            <div class="bl-site-analytics-basic-stats">
                <ul>
        ...
        if((isset($stats['top_searches']) && $stats['top_searches']) || (isset($stats['top_referers']) && $stats['top_referers'])) {
            $return .= '
                <div class="bl-site-analytics-extended-stats">';
                ...
                if((isset($stats['top_searches']) && $stats['top_searches']) || (isset($stats['top_referers']) && $stats['top_referers'])) {
                    ...
                    if(isset($stats['top_searches']) && $stats['top_searches']) {
                        ...
                        $return .= '
                            <div id="postcustomstuff" class="bl-site-analytics-searches">
                                <table class="wp-list-table widefat bl-site-analytics-table">
                                    <thead>
                                        <tr>
                    ...
                    if(isset($stats['top_referers']) && $stats['top_referers']) {
                        ...
                        $return .= '
                            <div id="postcustomstuff" class="bl-site-analytics-top-referrals last">
                                <table class="wp-list-table widefat bl-site-analytics-table">
                                    <thead>
                                        <tr>
  ...
  $current_settings = bl_ga_plus_get_settings( 'current' );
        ...
        if(!$this->cache)
            echo '<div id="bl-site-analytics-statistics-page"><div class="loading"><img alt="'.__( 'Loading...', $this->text_domain ).'" src="'.includes_url('images/spinner-2x.gif').'"/></div></div>';
        else
            echo '<input type="hidden" id="ga-pageview-color" class="ga-color-field pageview-color" name="ga_pageview_color" value="'.$current_settings['track_settings']['ga_pageview_color'].'"><input type="hidden" id="ga-visitors-color" class="ga-color-field visitors-color" name="ga_visits_color" value="'.$current_settings['track_settings']['ga_visits_color'].'"><input type="hidden" id="ga-unique-color" class="ga-color-field unique-color" name="ga_univisits_color" value="'.$current_settings['track_settings']['ga_univisits_color'].'"><div id="bl-site-analytics-statistics-page">'.$this->load_google_analytics().'</div>';
    }
    ...
    function google_analytics_statistics_page_html($stats) {
        ...
        $return = '
            <div class="wrap">
                <h2>
                    '.__( 'Site Analytics ', $this->text_domain ).$statistics_description.'
                    ...
                    <div id="post-body" class="metabox-holder columns-2">
                        <div id="postbox-container-1" class="postbox-container">
                            ...
                            <div aria-hidden="true" id="side-sortables" class="meta-box-sortables ui-sortable">
                                <div class="postbox bl-site-analytics-countries">
                                    <h3 class="hndle"><span>'.__( 'Visitors: Country', $this->text_domain ).'</span></h3>
                                    <div class="inside">
                                        <div id="bl-site-analytics-chart-countries" class="bl-site-analytics-chart"></div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div id="postbox-container-2" class="postbox-container">
                            <div id="normal-sortables" class="meta-box-sortables ui-sortable">
                                <div class="postbox">
                                    ...
                                    <div class="inside">
                                        <div aria-hidden="true" id="bl-site-analytics-chart-visitors" class="google_analytics_chart"></div>
                                        <div class="bl-site-analytics-basic-stats">
                                            <ul>
                                                <li><label>'.__( 'Visits', $this->text_domain ).'</label><span>'.esc_html($stats['visits']).'</span></li>
                                                <li><label>'.__( 'Unique Visitors', $this->text_domain ).'</label><span>'.esc_html($stats['unique_visitors']).'</span></li>
                            ...
                            <div id="normal-sortables" class="meta-box-sortables ui-sortable">
                                <div class="postbox">
                                    ...
                                    <div class="inside">
                                        <div id="postcustomstuff" class="bl-site-analytics-top_posts-pages">
                                            <table class="wp-list-table widefat bl-site-analytics-table">
                                                <thead>
                                                    <tr>
    ...
    function google_analytics_frontend_widget($args = array()) {
        if(!$this->cache)
            echo '<ul class="bl-site-analytics-frontend-widget"><li>'.__( 'Loading...', $this->text_domain ).'</li></ul>';
        else
            echo '<ul class="bl-site-analytics-frontend-widget">'.$this->load_google_analytics($args).'</ul>';
    }
...
$blitz_site_analytics_async_dashboard = new Blitz_Site_Analytics_Async_Dashboard();
