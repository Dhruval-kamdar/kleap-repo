...
class Blitz_Site_Analytics_Async_Frontend_Widget extends WP_Widget {
    //constructor
    function __construct() {
        global $blitz_site_analytics_async_dashboard;

        $this->text_domain = $blitz_site_analytics_async_dashboard->text_domain;
    ...
    function widget( $args, $instance ) {
        ...
        ob_start();
        $blitz_site_analytics_async_dashboard->set_up_ga_data();
        $blitz_site_analytics_async_dashboard->google_analytics_frontend_widget(array('number' => $number));
        $stats = ob_get_clean();
        ...
        if(!isset($google_analytics_frontend_widget_count) || $google_analytics_frontend_widget_count) {
            if(!isset($google_analytics_frontend_widget_count)) {
                wp_enqueue_script('blitz_site_analytics_async_frontend', $blitz_site_analytics_async_dashboard->plugin_url . 'blitz-site-analytics-async-files/ga-async-frontend.js', array('jquery'), 340);
                $blitz_site_analytics_async_dashboard->setup_script_variables('blitz_site_analytics_async_frontend');
            }
