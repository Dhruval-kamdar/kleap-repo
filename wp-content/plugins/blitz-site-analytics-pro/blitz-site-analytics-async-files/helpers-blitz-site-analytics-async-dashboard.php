 ...
 */
function bl_ga_plus_get_current_settings() {
	global $blitz_site_analytics_async;
	return $blitz_site_analytics_async->current_settings;
}

/**
 * Get the settings saved for the current blog
 */
function bl_ga_plus_get_current_site_settings() {
	global $blitz_site_analytics_async;
	return $blitz_site_analytics_async->settings;
}

/**
 * Get the settings for the whole network
 */
function bl_ga_plus_get_network_settings() {
	global $blitz_site_analytics_async;
	return $blitz_site_analytics_async->network_settings;
}

function bl_ga_plus_get_settings( $source ) {
	switch ( $source ) {
		case 'network': {
			$settings = bl_ga_plus_get_network_settings();
			break;
		}
		case 'site': {
			$settings = bl_ga_plus_get_current_site_settings();
			break;
		}
		default: {
			$settings = bl_ga_plus_get_current_settings();
			break;
		}
	}
	return $settings;
 ...
 */
function bl_ga_plus_update_setting( $option_name, $option_value, $source = 'site' ) {
	global $blitz_site_analytics_async;
	$is_network = $source === 'network' ? 'network' : '';
	if ( '' === $option_name ) {
		// Save all settings
		$blitz_site_analytics_async->save_options( $option_value , $is_network );
	}
	else {
		// Save just one setting
		$params = array( $option_name => $option_value );
		$blitz_site_analytics_async->save_options( $params , $is_network );
	}
}
 ...
 * @return array
 */
function bl_ga_plus_code_authenticate( $code ) {
	global $blitz_site_analytics_async_dashboard;

	$blitz_site_analytics_async_dashboard->google_client->authenticate( $code );
	$token_object = bl_ga_plus_get_google_token();
	bl_ga_plus_set_google_token( $token_object );

	$google_user_info = new GAPGoogle_Service_Oauth2($blitz_site_analytics_async_dashboard->google_client);
 ...
 * @return object
 */
function bl_ga_plus_get_google_token() {
	global $blitz_site_analytics_async_dashboard;
	$token = $blitz_site_analytics_async_dashboard->google_client->getAccessToken();
	if ( is_string( $token ) ) {
 ...
 */
function bl_ga_plus_set_google_token( $token ) {
	global $blitz_site_analytics_async_dashboard;
	if ( is_object( $token ) ) {
		$token = json_encode( $token );
	}
	$blitz_site_analytics_async_dashboard->google_client->setAccessToken($token);
}
 ...
 * @return array
 */
function bl_ga_plus_get_google_login_settings( $source = 'current' ) {
	$settings = bl_ga_plus_get_settings( $source );
 ...
 * @return array
 */
function bl_ga_plus_get_google_api_settings( $source = 'current' ) {
	$settings = bl_ga_plus_get_settings( $source );
 ...
 * @return bool
 */
function bl_ga_plus_is_google_login_logged_in( $source = 'current' ) {
	$settings = bl_ga_plus_get_google_login_settings( $source );
 ...
 * @return array|bool
 */
function bl_ga_plus_get_saved_token( $google_user_id ) {
	global $wpdb;
 ...
 */
function bl_ga_plus_get_saved_token_by_token_id( $token_id ) {
	global $wpdb;
 ...
 * @return int|mixed
 */
function bl_ga_plus_update_saved_token( $google_user_id, $token ) {
	global $wpdb;
	if ( is_object( $token ) ) {
		$token = json_encode( $token );
	}

	$db_token = bl_ga_plus_get_saved_token( $google_user_id );
	if ( $db_token['id'] ) {
 ...
 * @return int|mixed
 */
function bl_ga_plus_update_saved_token_by_token_id( $token_id, $token ) {
	global $wpdb;
	if ( is_object( $token ) ) {
		$token = json_encode( $token );
	}
 ...
 */
function bl_ga_plus_refresh_google_login_settings( $token_id, $type = 'network' ) {
	$token = bl_ga_plus_get_saved_token_by_token_id( $token_id );
	if ( ! $token ) {
		return;
	}
	$token_stringified = json_encode( $token );
	bl_ga_plus_update_setting( 'google_login',
		array(
			'token_id'      => $token_id,
 ...
 */
function bl_ga_plus_refresh_google_login_token( $google_login_settings, $source = 'site' ) {
	global $blitz_site_analytics_async_dashboard;
	...
	if(isset($orginal_token->refresh_token)) {
		$blitz_site_analytics_async_dashboard->google_client->refreshToken($orginal_token->refresh_token);
		$token_object = bl_ga_plus_get_google_token();
		...
		if ( ! empty( $google_login_settings['token_id'] ) ) {
			...
			bl_ga_plus_update_saved_token_by_token_id( $google_login_settings['token_id'], $db_token_object );
		}

		bl_ga_plus_update_setting( 'google_login', $google_login_settings, $source );
		bl_ga_plus_set_google_token( $token_object );
	}
}
 ...
 */
function bl_ga_plus_reset_google_login_settings( $source = 'network' ) {
	global $blitz_site_analytics_async;
	$blitz_site_analytics_async->save_options(array('google_login' => array()), $source);
	$blitz_site_analytics_async->save_options(array('google_api' => array()), $source);
}
 ...
 * @return array
 */
function bl_ga_plus_get_track_settings( $source = 'current' ) {
	$settings = bl_ga_plus_get_settings( $source );
