...
function gaa_load_callback () {
	if( jQuery('#bl-site-analytics-statistics-page').length > 0 || jQuery('#bl-site-analytics-widget').length > 0  ) {
			...
			if(ga_chart_visitors || ga_chart_countries)
				...
				if(ga_type == 'post' && ga.load_mode == 'soft') {
					jQuery("#load-post-stats").click(function(event) {
						...
						jQuery('#bl-site-analytics-widget #load-post-stats').hide();
						jQuery('#bl-site-analytics-widget .loading').show();
...
function ga_check_width() {
	var current_width = jQuery("#bl-site-analytics-chart-visitors").width();
...
function ga_load_google_analytics() {
	...
	jQuery.post(ajaxurl, {action: 'load_google_analytics', type: ga_type, post: post, network_admin: network_admin, date_range: ga_date_range }, function(data){ //post data to specified action trough special WP ajax page
		...
		if(data['error'] != true || ga_retry == 5) {
			jQuery('#bl-site-analytics-widget, #bl-site-analytics-statistics-page').html(data['html']);
...
function ga_load_charts(disable) {
	var ga_pageview = jQuery('#ga-pageview-color').val();
	var ga_visitors = jQuery('#ga-visitors-color').val();
	var ga_unique = jQuery('#ga-unique-color').val();
    console.log(ga_pageview, ga_visitors, ga_unique);
	...
	if(ga_chart_visitors && ga_chart_visitors.length > 1 && disable != 'chart_visitors') {
		...
		var options = {
			...
			colors: [ga_pageview, ga_visitors, ga_unique],
			chartArea: {top: 10, left: 60, width: '95%'},
			focusTarget: 'category'
			
		};

		var chart_visitors = new google.visualization.AreaChart(document.getElementById('bl-site-analytics-chart-visitors'));
		chart_visitors.draw(data, options);
		ga_width = jQuery("#bl-site-analytics-chart-visitors").width();
	}
	if(ga_chart_countries && disable != 'chart_countries') {
		...
		var chart_countries = new google.visualization.GeoChart(document.getElementById('bl-site-analytics-chart-countries'));
		chart_countries.draw(data, options);
		jQuery("#bl-site-analytics-statistics-page .bl-site-analytics-countries .inside").scrollLeft( 135 );
	}
}
