...
if(!class_exists('LicenceManagerGlobal')){
	class LicenceManagerGlobal{
		...
		public function licenseBlock() {
				echo '<div class="wrap">';
				echo '<h2>Site Analytics PRO Configuration</h2>';
				...
				if (isset($_REQUEST['reset_license'])) {
						...
						if( $license_key1 != '' ) {
							...
							if($response[0]){
								$this->updateOption($this->productField,'');
								$this->updateOption('siteanaly_license','');
								$this->updateOption('siteanaly_license_expired','');
							} else {
								$this->updateOption($this->productField,'');
								$this->updateOption('siteanaly_license','');
				...
				//~ $responsecheck = $this->license_check($license_key1,'status-check');
					...
					if( $license_key1 != '' ) {
						$adminSettings = new Blitz_Site_Analytics_Async();
						$responsecheck = $adminSettings->siteanaly_validLicense($license_key1);
					} else {
						$responsecheck = False;
					}
						...
						<table class="form-table">
								<tr>
										...
										<td>
											...
											<?php } ?>
											<?php if($responsecheck[0]){	echo '<img style="margin-left: 10px;vertical-align: middle;width: 23px;margin-top: -6px;" src="'.plugin_dir_url( __FILE__ ) . 'assets/image/greentick.png'.'">'; 		} ?>
											<?php if($license_key1 != '' && !$responsecheck[0]){	echo '<img style="margin-left: 10px;vertical-align: middle;width: 23px;margin-top: -6px;" src="'.plugin_dir_url( __FILE__ ) . 'assets/image/cross.png'.'" alt="cross">'; } ?>
											<p class="licenseKey"><?php if (isset($license_key_final) ) { echo $license_key_final; } ?></p>
										</td>
								</tr>
						</table>
						<p class="submit">
								<?php if($license_key1 != '' && $responsecheck[0]){ ?>
