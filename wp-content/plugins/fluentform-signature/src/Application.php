...
class Application {
	...
	public function adminHooks() {
		$this->fluentForm->addfilter( 'fluent_editor_components', function ( $components ) {
			return \FluentFormSignature\Component::push( $components );
		} );
		
		add_filter( 'fluent_editor_element_settings_placement', function ( $placement_settings ) {
		...
		add_filter('fluent_editor_element_customization_settings', function ($customization_settings) {
	...
	public function commonHooks() {
		...
		$this->fluentForm->addfilter( 'fluentform_insert_response_data',
		...
		$this->fluentForm->addfilter( 'fluentform_form_input_types', function ( $types ) {
			$types[] = 'signature';
			return $types;
		} );

        add_filter('fluentform_response_render_signature', function ($response, $field, $form_id, $isHtml = false) {
            if(!$response) {
                return '';
            }
	...
	private function publicHooks() {
		...
		add_action( 'fluentform_render_item_signature', function ( $signature, $form ) {
