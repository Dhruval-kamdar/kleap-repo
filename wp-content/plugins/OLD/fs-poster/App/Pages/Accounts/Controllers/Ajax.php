...
trait Ajax
{
	...
	public function delete_account ()
	{
		...
		if ( ! $check_account )
		{
			Helper::response( FALSE, fsp__( 'The account isn\'t found!' ) );
		}
		else
		{
			if ( $check_account[ 'user_id' ] != get_current_user_id() )
			{
				Helper::response( FALSE, fsp__( 'You don\'t have permission to remove the account! Only the account owner can remove it.' ) );
			}
		}
	...
	public function account_activity_change ()
	{
		...
		{
			Helper::response( FALSE, fsp__( 'Account not found or you do not have a permission for this account!' ) );
		}
	...
	public function make_account_public ()
	{
		...
		if ( $check_account[ 'user_id' ] != get_current_user_id() )
		{
			Helper::response( FALSE, fsp__( 'Only the account owner can make it public or private!' ) );
		}
	...
	public function add_wordpress_site ()
	{
		...
		$password = '(-F-S-P-)'.str_rot13( base64_encode( $password . '(-F-S-P-)' . time() ) );
	...
	public function get_tags_and_cats ()
	{
		...
		// post categories
		$post_cats          = mb_strlen( $search ) > 1 ? get_terms( [
			'taxonomy'   => 'category',
			'hide_empty' => FALSE,
			'search'     => $search
		] ) : get_terms( [ 'taxonomy' => 'category', 'hide_empty' => FALSE, 'orderby' => 'taxonomy', 'number' => 50 ] );
		$post_cats_children = [];
		...
		$tags_and_cats[] = [
			'text'     => fsp__( 'Post categories' ),
			'children' => $post_cats_children
		];

		// products
		$product_cats          = taxonomy_exists( 'product_cat' ) ? ( mb_strlen( $search ) > 1 ? get_terms( [
		...
		$tags_and_cats[] = [
			'text'     => fsp__( 'Product categories' ),
			'children' => $product_cats_children
		];

		// custom categories
		$custom_cats          = mb_strlen( $search ) > 1 ? get_terms( [
			'orderby'    => 'taxonomy',
			'hide_empty' => FALSE,
			'search'     => $search
		] ) : get_terms( [ 'orderby' => 'taxonomy', 'hide_empty' => FALSE, 'number' => 50 ] );
		$custom_cats_children = [];

		foreach ( $custom_cats as $cat )
		{
			if ( in_array( $cat->taxonomy, [ 'category', 'product_cat' ] ) )
			{
				continue;
			}
		...
		$tags_and_cats[] = [
			'text'     => fsp__( 'Custom categories' ),
			'children' => $custom_cats_children
		];
