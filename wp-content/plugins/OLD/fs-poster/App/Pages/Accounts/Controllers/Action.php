...
class Action
{
	...
	public function get_counts ()
	{
		...
		$nodes_list    = DB::DB()->get_results( DB::DB()->prepare( 'SELECT driver, COUNT( 0 ) AS _count FROM ' . DB::table( 'account_nodes' ) . ' WHERE account_id NOT IN ( SELECT id FROM ' . DB::table( 'accounts' ) . ' WHERE ( user_id = %d OR is_public = 1 ) AND blog_id = %d ) AND ( user_id = %d OR is_public = 1 ) AND blog_id = %d GROUP BY driver', [
			get_current_user_id(),
			Helper::getBlogId(),
			get_current_user_id(),
			Helper::getBlogId()
		...
		foreach ( $nodes_list as $nInf )
		{
			if ( isset( $fsp_accountsCount[ $nInf[ 'driver' ] ] ) )
			{
				$fsp_accountsCount[ $nInf[ 'driver' ] ][ 'total' ] = $nInf[ '_count' ];
				$fsp_accountsCount[ 'total' ]                      += $nInf[ '_count' ];
			}
		}
