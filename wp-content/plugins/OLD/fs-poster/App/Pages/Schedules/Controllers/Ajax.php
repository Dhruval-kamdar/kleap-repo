...
trait Ajax
{
	public function schedule_save ()
	{
		...
		{
			Helper::response( FALSE, fsp__( 'Too many posts are selected! You can select maximum 250 posts!' ) );
		}
	...
	public function schedule_posts ()
	{
		...
		else
		{
			...
			{
				Helper::response( FALSE, fsp__( 'Too many post selected! You can select maximum 250 posts!' ) );
			}
		}
	...
	public function schedule_get_calendar ()
	{
		...
		foreach ( $getPlannedDays as $planInf )
		{
			...
			while ( $cursorDayTimestamp <= $planEnd )
			{
				...
				if ( ( $cursorDayTimestamp >= $sleepTimeStart && $cursorDayTimestamp <= $sleepTimeEnd ) || ( $cursorDayTimestamp + 86400 >= $sleepTimeStart && $cursorDayTimestamp + 86400 <= $sleepTimeEnd ) )
				{
