...
//		if ( ! empty( $pErrors ) )
//		{
//			Helper::response( FALSE, fsp__( 'It\'s essential to have these permissions in the App: ' ) . implode( ' , ', $pErrors ) );
//		}
	...
	public static function authorize ( $appId, $accessToken, $proxy )
	{
		...
		else
		{
			...
			DB::DB()->query( 'DELETE FROM '.DB::table( 'account_node_status' ).' WHERE node_id IN ( SELECT id FROM '.DB::table( 'account_nodes' ).' WHERE account_id = "'.$fb_accId.'")' );
