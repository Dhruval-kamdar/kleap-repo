...
class InstagramApi
{
	...
	public static function sendPost ( $accountInfo, $type, $message, $link, $images, $video )
	{
		if ( $type == 'image' )
		{
			if ( ! extension_loaded( 'gd' ) )
			{
				return [
					'status'    => 'error',
					'error_msg' => fsp__( 'You should install and enable GD, libjpeg, libpng, and freetype libraries. ', [], FALSE )
					// <a href=\'https://www.fs-poster.com/documentation/commonly-encountered-issues#issue3\' target=\'_blank\'>Learn more.</a>
				];
			}
		...
		if ( $type == 'image' )
		{
			...
			catch ( Exception $e )
			{
				return [
					'status'    => 'error',
					'error_msg' => fsp__( 'Error: %s', [ $e->getMessage() ], FALSE )
				];
			}
		}
		else
	...
	public static function checkAccount ( $accountInfo )
	{
		...
		if ( self::isCookieMethod( $accountInfo ) )
		{
			...
			if ( ! $instagram_res )
			{
				$result = [
					'error'     => TRUE,
					'error_msg' => fsp__( 'The cookie value isn\'t valid! Please get the new one.' )
				];
			}
		}
		else
	...
	public static function handleResponse ( $result, $username, $password, $proxy )
	{
		...
		else if ( ! ( $result[ 'status' ] == 'ok' && ! empty( $result[ 'logged_in_user' ] ) ) )
		{
			...
			{
				$error_msg = fsp__( 'An error occurred while processing the request!' );
			}
