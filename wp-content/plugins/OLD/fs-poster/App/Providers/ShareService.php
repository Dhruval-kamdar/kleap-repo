	...
	public static function post ( $feedId, $secureShare = FALSE )
	{
		...
		else if ( $driver == 'instagram' )
		{
			...
			else
			{
				$res = [
					'status'    => 'error',
					'error_msg' => fsp__( 'Error! An image or video is required to share a post on Instagram.' )
				];
			}
		}
		else if ( $driver == 'linkedin' )
		{
			if ( $sendType != 'image' && $sendType != 'video' )
			{
				...
				if ( $pMethod === '2' )
				{
					$thumbnailPath = WPPostThumbnail::getPostThumbnailURL( $post_id );
		...
		{
			if ( Helper::getOption( 'reddit_posting_type', '1' ) == '3' )
			{
				$thumbnailPath = WPPostThumbnail::getPostThumbnailURL( $post_id );
		...
		else if ( $driver == 'tumblr' )
		{
			...
			{
				$tags = implode( ',', Helper::getPostTagsArray( $postInf ) );
			}
			else
			{
				$tags = '';
			...
			$res = Tumblr::sendPost( $nInf[ 'info' ], $sendType, $postTitle, $message, $link, $imagesLocale, $videoURLLocale, $accessToken, $accessTokenSecret, $appId, $proxy, $tags );
		}
		...
		else if ( $driver == 'google_b' )
		{
			if ( $sendType == 'video' )
			{
				$res = [
					'status'    => 'error',
					'error_msg' => fsp__( 'Google My Business doesn\'t support video type!' )
				];
			}
			else
			{
		...
		else if ( $driver == 'medium' )
		{
			...
			{
				$tags = Helper::getPostTagsArray( $postInf );
			}
			else
			{
				$tags = [];
			...
			$res = Medium::sendPost( $nInf[ 'info' ], $postTitle, $message, $accessToken, $proxy, $tags );
		}
		else if ( $driver == 'wordpress' )
		{
			...
			$nInf[ 'password' ] = substr( $nInf[ 'password' ], 0, 9 ) === '(-F-S-P-)' ? explode( '(-F-S-P-)', base64_decode( str_rot13( substr( $nInf[ 'password' ], 9 ) ) ) )[ 0 ] : $nInf[ 'password' ];
