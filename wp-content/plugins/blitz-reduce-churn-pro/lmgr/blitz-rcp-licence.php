<?php 
namespace BZ_RCP\Admin\License;
use BZ_RCP\Admin\RCPAdminSettings;

define('BZRCP_APP_API_URL',  'https://waas-pro.com/index.php');
...
class BZRCP_License {
 	...
 	public function init($productID=null) {
 		...
 		$this->productId = BZRCP_PRODUCT_ID;
 	...
 	public function bzrcp_license_status() {
 		...
 		//auto updater start
			Blitz_run_updater("https://waas-pro.com/index.php", 'blitz-reduce-churn-pro',BZRCP_PLUGIN_FILE,BZRCP_VERSION,$license_key1,$this->productId);
		//auto updater end
		
		return $this->bzrcp_license_check($license_key1,'status-check');
		
 	}



 	public function bzrcp_license_menu() {
	 		
	 		add_submenu_page(
		            'settings.php',
		            __('Reduce Churn License', 'my-plugin-domain'),
		            __('Reduce Churn License'),
		            'manage_options',
		            'glmlicense',
		            array($this, 'bzrcp_licenseBlock')
		        );    
 	...
 	public function bzrcp_license_check($license_key,$action) {
			...
			$request_uri    = BZRCP_APP_API_URL . '?' . http_build_query( $args );
 	...
 	public function bzrcp_licenseBlock() {
			...
			if (isset($_REQUEST['activate_license'])) {
					...
					$response = $this->bzrcp_license_check($license_key,'activate');
					echo $response[1];
					if($response[0]){
						if(is_multisite() ){
							if( is_plugin_active_for_network(BZRCP_PLUGIN_FILE) ) {
			...
			if (isset($_REQUEST['deactivate_license'])) {
					//~ $license_key = $_REQUEST[$this->productField];
					$response = $this->bzrcp_license_check($license_key1,'deactivate');
					echo $response[1];
					if($response[0]){
						if(is_multisite() ){
							if( is_plugin_active_for_network(BZRCP_PLUGIN_FILE) ) {
								update_site_option($this->productField, ''); 
								update_site_option('rcp_license_expired', ''); 
							} else {
								update_option($this->productField, ''); 
								update_option('rcp_license_expired', ''); 
							}
						} else {
							update_option($this->productField, ''); 
							update_option('rcp_license_expired', ''); 
					...
					if( $license_key1 != '' ) {
						...
						if($response[0]){
							if(is_multisite() ){
								if( is_plugin_active_for_network(BZRCP_PLUGIN_FILE) ) {
									update_site_option($this->productField, ''); 
									update_site_option('rcp_license', ''); 
									update_site_option('rcp_license_expired', ''); 
								} else {
									update_option($this->productField, ''); 
									update_option('rcp_license', ''); 
									update_option('rcp_license_expired', ''); 
								}
							} else {
								update_option($this->productField, ''); 
			...
			//~ $responsecheck = $this->bzrcp_license_check($license_key1,'status-check');
			
			if( $license_key1 != '' ) {
				$adminSettings = new RCPAdminSettings(BZRCP_PRODUCT_ID);
				$responsecheck = $adminSettings->bzrcp_valid_license();
			} else {
				$responsecheck = False;
			}
