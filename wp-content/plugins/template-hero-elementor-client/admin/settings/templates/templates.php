	...
	<div id="elementor-template-library-toolbar">
		<# if ( 'remote' === activeSource ) {
			...
			<div id="elementor-template-library-filter-toolbar-remote" class="elementor-template-library-filter-toolbar">
				...
				if ( config.categories.filter ) {
					var myNewArray = config.categories.filter(function(elem, index, self) {
						return index === self.indexOf(elem);
					});
				} else if( Object.values( config.categories ).filter ) {
					var myNewArray = Object.values( config.categories ).filter(function(elem, index, self) {
						return index === self.indexOf(elem);
					});
				}
				if ( myNewArray ) { #>
					<div id="elementor-template-library-filter" style="text-align: left !important;width:250px !important;padding-right:-20px !important">
						<select  id="elementor-template-library-filter-subtype" class="elementor-template-library-filter-select" data-elementor-filter="subtype">
							...
							<# myNewArray.forEach( function( category ) {
								var selected = category === elementor.templates.getFilter( 'subtype' ) ? ' selected' : '';
								#>
								<option value="{{ category }}"{{{ selected }}}>{{{ category.toLowerCase().replace(/\b[a-z]/g, function(letter) {return letter.toUpperCase();}) }}}</option>
							<# } ); #>
						</select>
					</div>
				<# } 
