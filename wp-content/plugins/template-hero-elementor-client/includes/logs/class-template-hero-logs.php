 ...
 * @author      J Hanlon
 * @category    Admin
 * @package     Template Hero/Logger
 * @version     1.0.0
*/
namespace TemplateHero\Plugin_Client\Logs;

class Template_Hero_Logger {
     ...
     * @return void
     */
    public function template_hero_elementor_admin_logs_settings_save() {
        if( isset( $_POST['template_hero_see_log'] ) ) {
            $template_hero_elementor_options    = array();

            $log_path = isset( $_POST['template_hero_log_select'] ) ? $_POST['template_hero_log_select'] : '';
            $uploads  = 'false';
            if( $log_path && $log_path != '' ) {
                $log_path     = template_hero_deslash( $log_path );
                $template_hero_elementor_options['template_hero_log_select'] =  $log_path;
                $template_hero_elementor_options = apply_filters( "template_hero_save_log_options", $template_hero_elementor_options );
                update_option( 'template_hero_elementor_log_options', $template_hero_elementor_options );
                $uploads  = 'true';
               
            }
            wp_safe_redirect( add_query_arg( 'logs-settings-updated', $uploads, $_POST['_wp_http_referer'] ) );
            exit;
        }
    }
    ...
    public static function get_logs_folder() {
        if( is_multisite() ) {
            ...
            $path = apply_filters( 'template_hero_get_logs_folder', self::get_uploads_folder() . "/template-hero-logs" . '/');
    ...
    public static function add( $handle, $message ) {
        ...
        do_action( 'template_hero_log_add', $handle, $message );
    }
    ...
    public function clear( $handle ) {
        ...
        do_action( 'template_hero_log_clear', $handle );
    }
}
