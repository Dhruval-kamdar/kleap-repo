...
if(!class_exists('LicenceManagerGlobal')){
	class LicenceManagerGlobal{
		...
		public function license_check($license_key,$action) {
				...
				$data           = wp_remote_post( $request_uri, array());
		...
		public function licenseBlock() {
				...
				if (isset($_REQUEST['reset_license'])) {
						...
						if( $license_key1 != '' ) {
							...
							if($response[0]){
								$this->updateOption($this->productField,'');
								$this->updateOption('lepelementor_license','');
								$this->updateOption('lepelementor_license_expired','');
							} else {
								$this->updateOption($this->productField,'');
								$this->updateOption('lepelementor_license','');
				...
				if( $license_key1 != '' ) {
					$adminSettings = new ElementorEditor();
					$responsecheck = $adminSettings->lepElem_validLicense($license_key1);
				} else {
					$responsecheck = False;
				}
