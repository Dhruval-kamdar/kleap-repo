	...
	<div id="<?php echo esc_attr( $element_id ); ?>" class="<?php echo esc_attr( implode( ' ', $class ) ); ?>">
		...
		if ( preg_match_all( '/(\*.*?\*)/is', $typed_text_safe, $entries ) ) {
			foreach ( $entries[0] as $entry ) {
				...
				$autotype = explode( ';', str_replace( '&amp;', '&', $text ) );
				$autotype = array_map( 'trim', $autotype );

				$typed_text_safe = str_replace( $entry, '<placeholder>', $typed_text_safe );
			}
		}
			echo str_replace( '<placeholder>', '<span class="thb-autotype-entry" data-thb-cursor="' . esc_attr( $cursor ) . '" data-thb-loop="' . esc_attr( $loop ) . '" data-strings="' . esc_attr( wp_json_encode( $autotype ) ) . '" data-speed="' . esc_attr( $typed_speed ) . '"></span>', $typed_text_safe );
		?>
