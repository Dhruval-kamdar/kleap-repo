<?php function thb_instagram_block( $atts, $content = null ) {
	...
	$el_class[] = 'row thb-instagram-row thb-instagram-block no-padding';
	$element_id = uniqid( 'thb-instagram-block-' );

	$username = $username === '' ? ot_get_option( 'instagram_username' ) : $username;

	if ( ! $username ) {
		 esc_html_e( 'Please check your Instagram username.', 'revolution' );
	}

	$instagram = thb_get_instagram_photos( 8, $username, $access_token );
	...
	<div id="<?php echo esc_attr( $element_id ); ?>" class="<?php echo esc_attr( implode( ' ', $el_class ) ); ?>">
		...
		if ( array_key_exists( 'data', $instagram ) ) {
			$i = 0; foreach ( $instagram['data'] as $item ) {
				?>
			<div class="small-6 medium-3 large-2 <?php echo esc_attr( $col ); ?> columns">
			...
			<?php } ?>
				...
				<div class="small-6 medium-3 large-2 thb-follower-column thb-light-column columns">
					...
					<span><?php esc_html_e( 'Followers', 'revolution' ); ?></span>
				</div>
				<div class="small-6 medium-3 large-2 thb-following-column thb-light-column columns">
					...
					<span><?php esc_html_e( 'Following', 'revolution' ); ?></span>
				</div>
			<?php } ?>
					<?php
					$i++; }
