...
function thb_wishlist_button() {
	if ( class_exists( 'YITH_WCWL' ) ) {
		...
		?>
		<div class="yith-wcwl-add-to-wishlist add-to-wishlist-<?php echo esc_attr( $product->get_id() ); ?> <?php echo esc_attr( $exists ? 'exists' : '' ); ?>">
...
function thb_remove_shop_sidebar() {
	if ( is_product() ) {
		remove_action( 'woocommerce_sidebar', 'woocommerce_get_sidebar' );
	}
}
add_action( 'template_redirect', 'thb_remove_shop_sidebar', 0 );
