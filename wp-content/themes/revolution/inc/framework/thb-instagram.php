...
function thb_get_instagram_photos( $number = 6, $username, $access_token = false ) {

	if ( $access_token && '' !== $access_token ) {
		$result = thb_get_instagram_photos_json( $number, $username, $access_token );
	} else {
		$result = thb_get_instagram_photos_html( $number, $username );
	}

	return $result;
}
function thb_get_instagram_photos_json( $number = 6, $username, $access_token ) {

	/* Check Access Token */
	if ( ! $access_token ) {
		$result['error'] = esc_html__( 'Please enter an Instagram Access Token.', 'revolution' );
		return $result;
	}
	/* Check Username */
	if ( ! $username ) {
		$result['error'] = esc_html__( 'Please enter an Instagram User.', 'revolution' );
		return $result;
	}

	if ( ! defined( 'THB_DEMO_SITE' ) && '8730733905.1677ed0.08da52b798f549c395fb4bfb4cb740e5' === $access_token ) {
		$result['error'] = esc_html__( 'Please enter your own Instagram Access Token.', 'revolution' );
		return $result;
	}
	...
	if ( ! $result ) {
		$instagram_self_link = add_query_arg(
			array(
		...
		$instagram_self_request = wp_safe_remote_get(
			$instagram_self_link,
			array(
				'timeout'     => 120,
				'httpversion' => '1.1',
		...
		$instagram_recent_photos = add_query_arg(
			array(
				'count'        => $number,
				'access_token' => $access_token,
			),
		...
		$instagram_recent_request = wp_safe_remote_get(
			$instagram_recent_photos,
			array(
				'timeout'     => 120,
				'httpversion' => '1.1',
				'redirection' => 10,
		...
		foreach ( $image_data->data as $key => $edge ) {
			...
			if ( isset( $edge->caption->text ) ) {
				...
				$result['data'][ $key ]['caption'] = wp_strip_all_tags( $text );
			}
		}
...
function thb_get_instagram_photos_html( $number = 6, $username ) {
	...
	if ( ! $result ) {
		...
		$remote_url = $proxies[ array_rand( $proxies ) ] . rawurlencode( $remote_url );
		...
		if ( ! $ins_data_json ) {
			return esc_html__( 'Instagram html data cannot be retrieved.', 'revolution' );
		}
		$image_data = false;
		...
		if ( $ins_data_json ) {
			...
			if ( ! isset( $instagram_json['entry_data']['ProfilePage'][0]['graphql']['user'] ) ) {
				return esc_html__( 'Instagram data is not set, please check the ID.', 'revolution' );
			}
			...
			if ( ! isset( $thb_user_data['edge_owner_to_timeline_media']['edges'] ) ) {
				return esc_html__( 'There are no images in your account or the account is private.', 'revolution' );
			}
			/* Live Count. */
			if ( ! empty( $thb_user_data['edge_owner_to_timeline_media']['edges'] ) ) {
				$image_data = $thb_user_data['edge_owner_to_timeline_media']['edges'];
			} else {
				$result['error'] = esc_html__( 'There are no images in your account or the account is private.', 'revolution' );
			}
			...
			$result['error'] = esc_html__( 'Instagram html data cannot be retrieved.', 'revolution' );
		}
		if ( $image_data ) {
