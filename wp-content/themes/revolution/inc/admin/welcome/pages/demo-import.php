	...
	<div class="thb-popup-box">
		...
		<figure>
			...
			<div class="thb-import-loading">
				...
				<div class="thb-loading-text"><?php esc_html_e( 'Page will refresh after the import is done.', 'revolution' ); ?></div>
			</div>
		</figure>
		<h3><?php esc_html_e( 'Import Content', 'revolution' ); ?></h3>
		<p><?php esc_html_e( 'You can select import data type you want.', 'revolution' ); ?></p>
		<form action="" method="post">
			<div class="thb-check-line">
				<div>
					<div>
						<input type="checkbox" name="ty-contents" id="ty-contents" class="child-opened thb-checked" checked>
						<label for="ty-contents"><?php esc_html_e( 'Contents', 'revolution' ); ?></label>
					</div>
					<div class="child-check done">
						<input type="checkbox" name="ty-contents-media" id="ty-contents-media" class="thb-checked" checked>
						<label for="ty-contents-media"><?php esc_html_e( 'Media Files (Thumbnail images, etc.)', 'revolution' ); ?></label>
					</div>
				</div>
				<div>
					<input type="checkbox" name="ty-theme-options" id="ty-theme-options" class="thb-checked" checked>
					<label for="ty-theme-options"><?php esc_html_e( 'Theme Options', 'revolution' ); ?></label>
				</div>
				<div>
					<input type="checkbox" name="ty-widgets" id="ty-widgets" class="thb-checked" checked>
					<label for="ty-widgets"><?php esc_html_e( 'Widgets', 'revolution' ); ?></label>
				</div>
			</div>
			<button type="submit" class="button button-primary"><?php esc_html_e( 'Import Selected', 'revolution' ); ?></button>
		</form>
	</div>
</main>
<div class="wrap about-wrap">
	...
	if ( ! class_exists( 'Revolution_plugin' ) ) {
		$thb_plugin = false;
	}
	?>
<div class="theme-browser thb-demo-import thb-content">
	...
	if ( ! $cond && $thb_plugin ) {
		...
		?>
		<div class="thb-error">
			<p><span class="dashicons dashicons-warning"></span> Please install <strong>Revolution Required Plugin</strong> on the previous tab first.</p>
		</div>
		<?php
	} else {
		...
		?>
			...
			if ( ( $i + 1 ) % 3 === 0 ) {
				...
				<div class="theme-actions">
					<span class="button button-primary import-opts-btn
					<?php
					if ( ! $cond ) {
						echo esc_attr( $disabled_class ); }
					?>
					" data-demo="<?php echo esc_attr( $i++ ); ?>">Import Options</span>
